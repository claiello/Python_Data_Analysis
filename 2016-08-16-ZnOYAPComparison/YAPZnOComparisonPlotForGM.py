...
ax1.set_title('ZnO:Ga brightness, \n ratio photons($<$ 409nm)/photons($>$ 409nm) = ' + str('{:0.1f}'.format(ratio)),fontsize=fsizepl)
...
plt.xlim([-1,3])
labels = ['Total', 'Blue', 'Red']
plt.xticks(major_ticks, labels, rotation='horizontal',fontsize=fsizepl)
plt.ylabel('CL counts, per pixel (MHz)',fontsize=fsizepl)
plt.ylim([0,0.5])
...
ax1.set_title('ZnO:Ga time-resolved decay',fontsize=fsizepl)
...
plt.ylabel('CL counts, per pixel (MHz)',fontsize=fsizepl)
plt.ylim([0,0.4])
plt.xlabel('Behaviour of e-beam: 0.12 ON - 3 OFF ($\mu$s)',fontsize=fsizepl)

multipage('GMComparisonYAPZnO.pdf',dpi=80)  
