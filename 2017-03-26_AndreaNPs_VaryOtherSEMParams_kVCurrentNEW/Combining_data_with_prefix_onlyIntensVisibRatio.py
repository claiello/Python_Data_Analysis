...
#    loadprefix = '../2017-03-17_AndreaNPs_VaryOtherSEMParams/' #USED THIS OLD DSET
#    nombre = 7
#    undex = 0
...
#    loadprefix ='../2017-03-26_AndreaNPs_VaryOtherSEMParams_kVCurrentNEW/'
#    nombre = 4
#    undex = 0
...
#    klklkk
#    
    #For current and kv, second vector is old dataSET, THE ONE FROM /2017-03-17_AndreaNPs_VaryOtherSEMParams/
    ####VISIB
    toplotpixel = [1.0,1.01514637055,1.01172723577,1.01363267797,1.00636743468,1.00841453996,1.00187643191]
    toplotcurrent = (np.array([1.0, 0.960757298473, 1.77680746, 1.47828419813])+np.array([1.0,0.887955652258,0.891502853426,1.23237112522]))/2.0
    toplotkv = (0*np.array([1.0,1.03455460536, 0.132204273338,0.653979207788])+np.array([1.0,1.12043553436,0.854931184789,1.11161402453]))/1.0

    ####RATIO
    toplotpixelratio = [1.0,1.04359332782,1.03347218482,1.03909173294,1.01794046721,1.0238252341,1.00523058474]
    toplotcurrentratio = (np.array([1.0, 1.02962482685, 0.513593008723, 0.680588612609])+ np.array([1.0,0.819496980284,0.824408154554,1.67355621548]))/2.0
    toplotkvratio = (0*np.array([1.0, 1.04063435548,0.423854098366,0.692967021064])+np.array([1.0,1.45334961564,0.704557498843,1.4086256255]))/1.0
    ...
    toploterrcurrentV = [0.000357074573367,0.00030897275162, 0.000952409904198, 0.000402650795032]
    toploterrkvV = [0.000177227711882, 0.000181610030352,0.000208250131899,0.000201579720462] 
    ...
    toploterrcurrentR = [0.000266921047228, 0.000242999401171, 0.000451188257358,0.00015690979144]
    toploterrkvR = [0.000202431531621, 0.000214228589298,0.00010049204124,0.000161106318067] 
    
    toplotpixelgreen = [1.0, 0.924782717691, 0.942459358192, 0.905744005851, 0.93076602506, 0.924890785283, 0.952211321734]
    toplotpixelred = [1.0, 0.965097086283, 0.974005544548, 0.941151120751, 0.947464414598, 0.946926536937, 0.957191956056]
...
#    klklklk
    ...
    #delete 10mum aperture from current bc noise limited
    Current = np.delete(Current, 2)
    toplotcurrent= np.delete(toplotcurrent, 2)
    toploterrcurrentV = np.delete(toploterrcurrentV, 2)
    toplotcurrentratio= np.delete(toplotcurrentratio, 2)
    toploterrcurrentR = np.delete(toploterrcurrentR, 2)
    ...
    p1 = host.plot( Pixel_size, toplotpixelgreen, marker='^',markersize=12,linestyle='',color='g', markeredgecolor='None')
    p1 = host.plot( Pixel_size, toplotpixelred,   marker='^',markersize=12,linestyle='',color='r', markeredgecolor='None')
