...
Pixel_size =[1.0e-07, 1.0e-07, 8.5e-08, 8.5e-08, 2.8e-07, 1.1e-07, 1.6e-07, 9.1e-08] #as given by accompanying text file np.ones([len(name)])*99
Ps = [100, 100, 85,85,280,110,160,91] #[56,56,56,56,56,58,59] np.ones([len(name)])*99
lag =  np.ones([8])*100 #np.ones([7])*100 #[500, 50,50,50,50,50,50] np.ones([len(name)])*99
frames =  [1,20,1,20,1,1,1,1] #[20, 50,50,50,100,100,100] np.ones([len(name)])*99
obs = ['Region I', 'Region II','Region III',  'Region IV', 'Region V', 'Region VI', 'Region VII', 'Region VIII'] # np.ones([len(name)])*99
listofindices = np.arange(0,len(name)) #[0,13]# #14is a TR
indexh = 0
kv = [5,5,5,5,8,8,10,10,10]

for index in [5]: #[1,3,5,7,13,14,15,17]: #good ones #listofindices:
    ...
    plot_2_channels_divide(se_dset_cut,blue_dset_cut, red_dset_cut, Pixel_size[indexh], title, scinti_channel, sample_channel, length_scalebar, scalebar_legend,unit,work_red_channel=True)
