...
nominal_time_on = 99 #time during which e-beam nominally on, in mus
...
#for index in listofindex:
    ...
    se = np.load('ISEchannel.npz') 
    segmm = np.load('ISEchannelGMM.npz') 
    red = np.load('IRedbright.npz') 
    blue = np.load('IBluebright.npz') 
    ...
    titulo = description[index] + ' (20kV, 30$\mu$m aperture, 2$\mu$s time bins, ' + str(Ps)+ 'nm pixels, red/blue: $>$715nm/$<$650nm)'
    ...
    plt.imshow(np.average(se['data'],axis=0),cmap=cm.Greys_r)
    ...
    im = plt.imshow(np.average(hlpse,axis=0),cmap=cm.Greys) #or 'OrRd'
    sbar = sb.AnchoredScaleBar(ax1.transData, length_scalebar_in_pixels, scalebar_legend, style = 'bright', loc = 4)
    ax1.add_artist(sbar)
    plt.axis('off')
    ...
    hlp[np.isnan(hlp)] = 1.0 #outside #consider all light
    
    # OUTSIDE
    hlpd = np.copy(segmm['bright'])
    hlpd[~np.isnan(hlpd)] = 0.0 
    hlpd[np.isnan(hlpd)] = 0.0 #consider all light
    ...
    ax1.axvspan(2.0+0.3,2.0+198.0+0.3, alpha=0.25, color='yellow')
    unit = 'kHz'
    ...
    if True is False:
        ...
        initbin = (99+1+1)-1 #init bin for decay
        backgdinit = 1
        ### 700ns /40ns = 7. ....
...
#    ee_array_blue[index] = ee2  
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLred[:,backgdinit:initbin,:,:]*hlp,axis=(1,2,3)),c='r', label='',linestyle='None', marker='o',markersize=4) #in mus, in MHz
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue[:,backgdinit:initbin,:,:]*hlp,axis=(1,2,3)),c='b', label='',linestyle='None', marker='o',markersize=4) #in mus, in MHz
