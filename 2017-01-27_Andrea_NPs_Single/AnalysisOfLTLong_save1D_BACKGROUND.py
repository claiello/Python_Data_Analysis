...
nametr = ['2017-01-27-1855_ImageSequence__500.000kX_10.000kV_30mu_16']
prefix =''
let = ['back']
index = 0
...
#    se1_dset2 = np.array(se1_dset0[0:No_experiments[0],0:finalx,:],dtype=np.float32)
    ...
    file20    = h5py.File(nametr[0] + '.hdf5', 'r')  
...
#    klklk
    ...
    mycode = str(let[index]) + 'SEchannelD = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) + 'SEchannelD', data = se1_dset_reg)
    
#    mycode = str(let[index]) + 'RedbrightD = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez(str(let[index]) +'RedbrightD', data = red1_dset_reg_all)
#    
#    mycode = prefix + str(let[index]) + 'RED1DD = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez(prefix + str(let[index]) +'RED1DD', data = np.nanmean(red1_dset,axis = (0,2,3)))
##    
    mycode = str(let[index]) + 'BluebrightD = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) +'BluebrightD', data = red1_dset_reg_all)
    
    mycode =str(prefix + let[index]) + 'BLUE1DD = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(prefix + str(let[index]) +'BLUE1DD', data = np.nanmean(red1_dset,axis = (0,2,3)))
##    
    ###############################################################################
    ###############################################################################
    ###############################################################################
