...
No_experiments = 3*np.ones([1])
                  
nametr = ['2017-03-29-1539_ImageSequence__10.000kX_2.000kV_30mu_14']

description = ['MOCHA'] # (20kV, 30$\mu$m, ' + str(Ps[index]) + 'nm pixels, ' + str(No_experiments[index]) + 'expts., InLens registered)'] #, \n' #+ obs[index] ']    
               
kv = [2]

#nominal Temps
#nominal Temps
let = ['onlyone'] #no of pixels
Pixel_size = [37.2]
...
red_decay_array = np.zeros([len(nametr),200])
blue_decay_array = np.zeros([len(nametr),200])
...
#    print('after plot 3, bef ft') 
    ...
    red_decay_array[index,:] = np.average(dataALLred[:,:,:,:]*hlp,axis=(0,2,3))
    #blue_decay_array[index,:] = np.average(dataALLblue[:,:,:,:]*hlp,axis=(0,2,3))
