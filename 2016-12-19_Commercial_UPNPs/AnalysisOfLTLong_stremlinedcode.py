...
for index in listofindex:
    ...
    titulo = description + ' (10kV, 30$\mu$m aperture, 1$\mu$s time bins, ' + str(Ps)+ 'nm pixels, red: all light)'
...
#    cb2 = plt.colorbar(im, cax = axColor, orientation="horizontal",label='Photon counts ' + unit) 
    ...
    if True is False:
        ...
        initbin = (201)-1+2 #init bin for decay
        backgdinit = 50
        ### 700ns /40ns = 7. ....
...
#    ee_array_blue[index] = ee2  
    ...
    del datared, datared_init#,datablue, datablue_init, 
    gc.collect()
    dataALLred = red['data'][:,:,:,:]
