#commercial UC NPS
#### i actually think there is no blue channel!!!!!!
...
totalpoints = 1200 #total number of time-resolved points
### data
...
nametr = ['2016-12-19-1153_ImageSequence_commercialNP_200.000kX_10.000kV_30mu_10',
          '2016-12-19-1223_ImageSequence_commercialNP_50.000kX_10.000kV_30mu_11',
          '2016-12-19-1248_ImageSequence_commercialNP_50.000kX_10.000kV_30mu_12',
          '2016-12-19-1318_ImageSequence_commercialNP_53.034kX_10.000kV_30mu_13']

description = ['Commercial UCNPs LiYF4:Er'] # (20kV, 30$\mu$m, ' + str(Ps[index]) + 'nm pixels, ' + str(No_experiments[index]) + 'expts., InLens registered)'] #, \n' #+ obs[index] ']    
...
#for index in np.arange(0,4):
    ...
    red0_dset  = file2['/data/Counter channel 2 : PMT blue/PMT blue time-resolved/data']#10 Scany points X 10 frames x 150 tr pts x250 x 250 pixels
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    del se1_dset, red1_dset#, blue1_dset
    file2.close()
   # file2.close()
    gc.collect()
...
#    blue1_dset_cut_all = np.array(blue1_dset_cut_all, dtype=np.float32)
    ...
    mycode = str(let[index]) + 'RANDSEchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) + 'RANDSEchannel', data = se1_dset_cut)
    
#    mycode = str(let[index]) + 'RANDRedbright = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez(str(let[index]) +'RANDRedbright', data = red1_dset_cut_all)
    
    mycode = str(let[index]) + 'RANDBluebright = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) +'RANDBluebright', data = red1_dset_cut_all)
    
    mycode = str(let[index]) + 'RANDILchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) + 'RANDILchannel', data = il1_dset_cut)
#    
    ...
    if do_gmmse_dset:
        ...
        mycode = str(let[index]) +'RANDSEchannelGMM = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez(str(let[index]) +'RANDSEchannelGMM', bright = gmmse_se1_bright_dset, means = means, covars = covars, weights = weights)
...
#        calcdecay_each(gmmse_red1_darkse_dset_for_4D[:,start_of_transient::,:,:]/darkse_pct, time_detail= Time_bin*1e-9*fastfactor,titulo='Cathodoluminescence rate decay, bi-exponential fit, \n ' + titulo ,single=False,nominal_time_on=nominal_time_on,fastfactor=fastfactor,other_dset1=red1_dset_cut_all[:,start_of_transient::,:,:]/1.0 ,other_dset2=gmmse_red1_brightse_dset_for_4D[:,start_of_transient::,:,:]/brightse_pct,init_guess=init_guess)    
    ...
    mycode = str(let[index]) +'RANDZZZBlueDecay = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) +'RANDZZZBlueDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    
#    mycode = str(let[index]) +'RANDZZZRedDecay = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez(str(let[index]) +'RANDZZZRedDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
...
##files below exist 
b_array_red = np.zeros(4)
be_array_red = np.zeros(4)
e_array_red = np.zeros(4)
ee_array_red = np.zeros(4)
b_array_blue = np.zeros(4)
be_array_blue = np.zeros(4)
e_array_blue = np.zeros(4)
ee_array_blue = np.zeros(4)
blue_int_array = np.zeros(4)
red_int_array = np.zeros(4)
cumu_blue = np.zeros([4,1])
cumu_red = np.zeros([4,1])
il_data = np.zeros([4,1])

listofindex =np.arange(0,4) # #[0,1,2]  [5] 
#index = 0
#if index is 1:
for index in listofindex:
    ...
    ### 593 dichroic
    se = np.load(str(let[index]) +'RANDSEchannel.npz') 
    segmm = np.load(str(let[index]) +'RANDSEchannelGMM.npz') 
    red = np.load(str(let[index]) +'RANDRedbright.npz') 
    blue = np.load(str(let[index]) +'RANDBluebright.npz') 
    il = np.load(str(let[index]) +'RANDILchannel.npz') 
...
#        hlp[~np.isnan(hlp)] = 1.0
    ...
    hlp = segmm['bright']   #1.0
    hlp[~np.isnan(hlp)] = 1.0    #### To consider only light from signal pixels, I think I need a 0 HERE!!!!!!!! 
    ...
    #hlp = 1.0
    plt.plot(np.arange(0,1200)*Time_bin/1e3,np.nanmean(red['data']*hlp,axis = (0,2,3)),c='r',label='Red photons ($>$ 593nm)',lw=3) #in mus, in MHz
    #plt.plot(np.arange(0,1200)*Time_bin/1e3,np.nanmean(blue['data']*hlp,axis = (0,2,3)),c='b',label='Blue photons ($<$ 593nm)',lw=3) #in mus, in MHz
    ...
    if True is False:
        ...
        initbin = 200  ####"correct" IS 200 OR 201 (BC OF 300NS DELAY)
        timeinit = 50
...
#    else:
    ...
    init_guess = [np.nanmean(hlp_red[0,:,:])+1.0, 15.0, cguessred, np.nanmean(hlp_red[-1000:-500,:,:])+1.0, 200.0 ] #RED
    init_guess2 = [np.nanmean(hlp_blue[0,:,:])+1.0, 15.0,cguessblue, np.nanmean(hlp_blue[-1000:-500,:,:])+1.0, 200.0 ] #BLUE 
    ...
    b,e,be,ee,b2,e2,be2,ee2 = calcdecay_subplot_nan(datared*hlp, time_detail= Time_bin*1e-9*fastfactor,titulo='Cathodoluminescence rate decay, bi-exponential fit, \n ' + titulo ,single=False,other_dset2=None ,other_dset1=None,init_guess=init_guess,unit='kHz',init_guess2=init_guess2) #,error_array=error_arrayr, error_array2=error_arrayb)    
   ...
   # major_ticks = [5,10,15,20]
    ...
    multipage_longer('RANDZZZZ-'+ let[index] + '.pdf',dpi=80)
...
#####FIG WITH SUMMARY   
KKKKKK
#!!!!!!!!!! ALSO PRINT BACKGROUND. RED/BACKGROUND AND BLUE/BACKGROUND
fastfactor = 1
