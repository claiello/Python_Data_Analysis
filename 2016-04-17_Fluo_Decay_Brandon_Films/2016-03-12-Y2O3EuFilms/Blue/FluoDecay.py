...
total_exp_time = 480e-6
No_frames = 240 #max 240
No_decaypts = No_frames
time_detail = 2 # in microsec; 1/expt clock rate
No_specimen = 3

hdf5_file_name = ['Place1.hdf5','Place2.hdf5','Place3.hdf5']
plot_title = 'Blue Y2O3:Eu film transient fluorescence (2kV, 30$\mu$m, 7.5kX, time detail = 2$\mu$s)'
...
(a,b,c,d,e) = fitexp(x_array[:-1]*time_detail,sum_grana_blue[1:]/1000.0/No_specimen,single=False)       

plt.plot(x_array[:-1]*time_detail,(sum_grana_blue[1:])/1000.0/No_specimen,'bo',label='Average decay, $ \\tau_1 $ = ' + str("{0:.2f}".format(b)) + '$\mu$s; $ \\tau_2 $ = ' + str("{0:.2f}".format(e)) + '$\mu$s',markersize=(kkk+1)+2)     
