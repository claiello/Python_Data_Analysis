...
#for index in np.arange(12,12):
    ...
    file1    = h5py.File('DoubleZoom.hdf5', 'r')  
    file2   = h5py.File('DoubleZoomTR.hdf5', 'r') 
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    mycode = 'SEchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('SEchannel', data = se1_dset_cut)
    
#    mycode = 'Redbright = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez('Redbright', data = red1_dset_cut_all)
    
    mycode = 'Bluebright = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('Bluebright', data = red1_dset_cut_all)
    ...
    if do_gmmse_dset:
        ...
        mycode = 'SEchannelGMM = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('SEchannelGMM', bright = gmmse_se1_bright_dset, means = means, covars = covars, weights = weights)
...
##files below exist
se = np.load('SEchannel.npz') 
segmm = np.load('SEchannelGMM.npz') 
red = np.load('Redbright.npz') 
blue = np.load('Bluebright.npz') 
...
local_maxi = peak_local_max(distance, num_peaks = 2, indices = False, footprint=np.ones((10,10)),labels=image) #footprint = min dist between maxima to find
...
multipage_longer('ZZZ-Double_plot.pdf',dpi=80)
