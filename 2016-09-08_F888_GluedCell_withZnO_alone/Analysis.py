...
Pixel_size =  [1.5e-07, 7.0e-08, 9.5e-08, 9.5e-08] #as given by accompanying text file
Ps = [150, 70, 95, 95] #[56,56,56,56,56,58,59]
lag = [20,20,100,100] #[500, 50,50,50,50,50,50]
frames = [5,5,5,20] #[20, 50,50,50,100,100,100] 
obs = ['Region I', 'Region II','Region III',  'Region IV']
index = 1
listofindices = np.arange(0,len(name)-1) #[0,13]# #14is a TR
indexh = 0

for index in [8,9,12,13]: #good ones #listofindices:
    ...
    #plt.close("all")
    title =  'F888 + PMMA on ZnO:Ga (2kV, 30 $\mu$m, ' + str(Ps[indexh]) + 'nm pixels, ' + str(lag[indexh]) + '$\mu$s lag per pixel, ' + str(frames[indexh]) + 'expts., SE registered), \n' + obs[indexh] 
