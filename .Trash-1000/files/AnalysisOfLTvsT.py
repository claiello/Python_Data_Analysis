...
#for index in [3,6]: #np.arange(0,10):
    ...
    red1_dset = red0_dset[index,:,:,:,:]#.reshape((No_experiments[index],:,:,:],150,250,250))
    blue1_dset = blue0_dset[index,:,:,:,:]#.reshape((No_experiments[index],:,:,:],150,250,250))
...
#    ###MOCK REGISTRATION, QUICK FOR HACK ####################################!!!!!!!!!!!!!!!!!!!!
    ...
    il1_dset_reg = il1_dset # DONT average over frames axis
    
#    blue1_dset_reg = np.average(blue1_dset, axis=0)
#    blue1_dset_reg_all = np.array(blue1_dset)
    il1_dset_reg = np.array(il1_dset_reg, dtype=np.float16)
...
#    
    if index is 0: #needs to be reworked out
       #pass
       init_guess = [60.3, 0.047, 40.0, 40.0,2.0 ] #RED
       init_guess2 = [94.42, 0.007, 3.0, 3.0,10.0 ] #BLUE
          
    if index is 1:
        init_guess = [144.0, 0.03, 60.0,70.0, 10.0 ]
        init_guess2 = [680.0, 0.05, 90.0,110.0, 10.0 ]
...
#        
    if index is 8:
        init_guess = [20.0,0.1, 20.0,1.0, 15.0 ] #RED IS BAD
        init_guess2 = [77.0,0.04, 2.0,32.0, 6.3 ]
        
    if index is 9:
         init_guess = [15.0,0.09, 9.0,10.0, 5.0]
         init_guess2 = [70.0,0.03, 40.0,40.0, 15.0]
...
fig41.set_size_inches(1200./fig40.dpi,900./fig40.dpi)
plt.rc('text', usetex=True)
plt.rc('font', family='serif')
plt.rc('font', serif='Palatino')    
plt.suptitle("Results as a function of temperature",fontsize=fsizetit)
