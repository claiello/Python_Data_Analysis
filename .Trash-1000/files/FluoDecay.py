...
def fitexp(x,y,single,my_color,my_edgecolor,my_facecolor,init_guess=None,plot_error=True,do_plot=True):
    ...
    if init_guess is not None:
        ...
        if single == False:
            print('HERE')
            params.add('e', init_guess[3], min=init_guess[3]/10.0,max=init_guess[3]*10.0)
            params.add('d', init_guess[4], min=init_guess[4]/10.0,max=init_guess[4]*10.0) 
    else:
...
#START OF LOOP
def calcdecay(blue_dset,time_detail,titulo,single,other_dset1=None, other_dset2=None, init_guess=None): #blue_dset has the transient data only
    No_specimen = 1
    ...
    if single == False:
        (a,b,c,d,e,ae,be,ce,de,ee) = fitexp(x_array/1e-6,np.average(blue_dset,axis=(1,2))/No_specimen,single=False, my_color='y',my_edgecolor='#ffff32', my_facecolor='#ffff66',init_guess=init_guess,plot_error=True)       
    #e stands for "error"  
...
def calcdecay_series(blue_dset,time_detail,titulo,single,nominal_time_on,fastfactor,other_dset1=None, other_dset2=None, init_guess=None): #blue_dset has the transient data only
    ...
    for jj in np.arange(blue_dset.shape[0]):
        ...
        # do fit, here with leastsq model
        if single == False:
            (a,b,c,d,e,ae,be,ce,de,ee) = fitexp(x_array/1e-6,np.average(np.average(blue_dset[0:jj+1,:,:,:],axis = 0),axis=(1,2))/No_specimen,single=False, my_color='y',my_edgecolor='#ffff32', my_facecolor='#ffff66',init_guess=init_guess,plot_error=True,do_plot=False)       
            tau1[jj] = b
            tau1_error[jj] = be
            tau2[jj] = e
            tau2_error[jj] = ee
...
def calcdecay_each(blue_dset,time_detail,titulo,single,nominal_time_on,fastfactor,other_dset1=None, other_dset2=None, init_guess=None): #blue_dset has the transient data only
    ...
    for jj in np.arange(blue_dset.shape[0]):
        ...
        # do fit, here with leastsq model
        if single == False:
            (a,b,c,d,e,ae,be,ce,de,ee) = fitexp(x_array/1e-6,np.average(blue_dset[jj,:,:,:],axis=(1,2))/No_specimen,single=False, my_color='y',my_edgecolor='#ffff32', my_facecolor='#ffff66',init_guess=init_guess,plot_error=True,do_plot=False)       
            tau1[jj] = b
            tau1_error[jj] = be
            tau2[jj] = e
            tau2_error[jj] = ee
