...
### data
name = ['Er2A.hdf5', 'Er2B.hdf5','Er20A.hdf5', 'Er20B.hdf5','Er40A.hdf5', 'Er40B.hdf5','Er40CA.hdf5', 'Er40CB.hdf5','Er80A.hdf5', 'Er80B.hdf5', 'OleicAcid.hdf5']
name_str = ['Er2A','Er2B','Er20A','Er20B','Er40A','Er40B','Er40CA','Er40CB','Er80A','Er80B','OleicAcid']
...
Pixel_size = [2.7912499e-08,2.7912499e-08,2.7912499e-08,2.7912499e-08,2.7912499e-08,2.7912499e-08,2.7912499e-08,2.7912499e-08,2.7912499e-08,2.7912499e-08,2.7912499e-08]
Ps = [28,28,28,28,28,28,28,28,28,28,28] #pixel size in nm, the numbers above with round nm precision
...
#original
cut_at_beginning = 79
dset_decay = np.zeros([11,totalpoints - 79])
mean_brightness = np.zeros([11])

#for index in np.arange(11):   #11):
titulo =  'Er core-shell/core-only aggregates (4kX, 2kV, 30$\mu$m, 40ns time bins, 28nm pixels)'
...
#for index in np.arange(12,12):
    ...
    titulo =  'Er ' +  na[index] + '$\%$ core-shell aggregates (4kX, 2kV, ' +  ap[index] + '$\mu$m, 40ns time bins, ' + str(Ps[index]) + 'nm pixels), sample ' + nl[index]
...
#    mycode = 'ZZZZZZblueALLexptsdecayEr40CSA = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez('ZZZZZZblueALLexptsdecayEr40CSA', data = red1_dset)
#    
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    ###############################################################################
    ###############################################################################
    dset_decay[index] = np.average(gmmse_red1_darkse_dset, axis = (1,2))    
    mean_brightness[index] = np.average(gmmse_red1_darkse_dset, axis = (0,1,2))    
...
#    last_pt_offset = -1 #sometimes use -1, last point, but sometimes this gives 0. -10 seems to work
...
#    mycode = 'ZZZZZZbluedecayEr40CSA = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez('ZZZZZZbluedecayEr40CSA', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
...
#        np.savez('ZZZZZZEr60' + str(index), data = np.average(gmmse_red1_darkse_dset_for_4D/darkse_pct, axis=(1,2,3)))
    ...
    #multipage('ZZZ' + name_str[index] +  '.pdf',dpi=80)
...
#mycode = 'Alldecays = tempfile.NamedTemporaryFile(delete=False)'
#exec(mycode)
#np.savez('Alldecays', dset_decay = dset_decay, mean_brightness = mean_brightness)
#plot_expt_by_expt_behaviour(titulo + r', all pixels', dset_decay, Time_bin, nominal_time_on,fastfactor,'r',major_ticks,dark_dset=None,plot_dark=False,unit='kHz') #pass titulo as well
#multipage('ZZZZAlldecays' + name_str[index] +  '.pdf',dpi=80)
#    
data = np.load('Alldecays.npz') 
#data['dset_decay']   
...
for k in range(6):
    ...
    plt.plot(np.arange(0,121)*Time_bin/1e3,new[k,:],c=c,label=lab[k],lw=3) #in mus, in MHz
    #else:
...
plt.ylabel("Average luminescence \n of each time bin (" + unit + ")",fontsize=20)
...
ax2 = plt.axes([.2, .7, .1, .1])# axisbg='y')
color=iter(cm.rainbow(np.linspace(0,1,6)))
c=next(color)#purple
c=next(color)#blue
c=next(color)#cyan
c=next(color)#green
ax2.plot(np.arange(0,10)*Time_bin/1e3,new[3,0:10],c=c,lw=3)
c=next(color)#orange
ax2.plot(np.arange(0,10)*Time_bin/1e3,new[4,0:10],c=c,lw=3)
c=next(color)#red
ax2.plot(np.arange(0,10)*Time_bin/1e3,new[5,0:10],c=c,lw=3)
plt.setp(ax2, yticks=[1,2,3], xticks=[])
ax2.spines['right'].set_visible(False)
ax2.spines['top'].set_visible(False)
ax2.xaxis.set_ticks_position('bottom')
ax2.yaxis.set_ticks_position('left')
ax2.axvspan(0.04, 0.16, alpha=0.25, color='red')
...
fastfactor = 1
last_pt_offset = -1 #sometimes use -1, last point, but sometimes this gives 0. -10 seems to work
init_guess = [np.average(dataall['data']), 1.0, np.average(dataall['data'][last_pt_offset,:,:]), np.average(dataall['data'][4,:,:]), 0.1] #e init was 0.5
b,e,be,ee = calcdecay_subplot(dataall['data'], time_detail= Time_bin*1e-9*fastfactor,titulo='Cathodoluminescence rate decay, bi-exponential fit, \n ' + titulo ,single=False,other_dset1=datared['data'] ,other_dset2=datablue['data'],init_guess=init_guess,unit='kHz')    
plt.xlim([0,2])
major_ticks0 = [1,2]
plt.ylabel("Average luminescence \n of each time bin (" + unit + ")",fontsize=20)
...
dataALLred = np.load('ZZZZZZredALLexptsdecayEr40CSA.npz') 
dataALLblue = np.load('ZZZZZZblueALLexptsdecayEr40CSA.npz') 
dataALLall = np.load('ZZZZZZALLALLexptsdecayEr40CSA.npz') 

plt.plot(np.arange(1,51)*nominal_time_on*fastfactor,np.average(dataALLall['data'],axis=(1,2,3)),c='k', label='From all pixels',linestyle='None', marker='o') #in mus, in MHz
plt.plot(np.arange(1,51)*nominal_time_on*fastfactor,np.average(dataALLblue['data'],axis=(1,2,3)),c='b', label='From blue photons ($<$ 458nm)',linestyle='None', marker='o',markersize=4) #in mus, in MHz
plt.plot(np.arange(1,51)*nominal_time_on*fastfactor,np.average(dataALLred['data'],axis=(1,2,3)),c='r', label='From red photons ($>$ 458nm)',linestyle='None', marker='o',markersize=4) #in mus, in MHz
