...
#
name_str = [s + 'ALL' for s in name_str]
...
#original
#for index in np.arange(10,11):   #11):
...
#for index in np.arange(12,12):
    ...
    if index is 0:
        file2    = h5py.File('2016-08-06-1537_ImageSequence_Er2CSSE_4.000kX_2.000kV_30mu_1.hdf5', 'r')  #for index = 0 ONLY
        se1_dsetONE   = file2['/data/Analog channel 1 : SE2/data'] #1 frames x250 x 250 pixels
    else:
    ...
    blue =  file1['/data/Counter channel 2 : PMT blue/PMT blue time-resolved/data']#50 frames x 200 tr pts x250 x 250 pixels
    #cut part of frames with ebeam off, here 25 first points
    
    cut_at_beginning = 79
    red1_dset = red1_dset[:,cut_at_beginning::,:,:]
    blue = blue[:,cut_at_beginning::,:,:]
    ...
    #no experiments to consider
    if index is 0:
        se1_dset = np.zeros([No_experiments[index]+1,250,250])
        for jj in np.arange(0,No_experiments[index]+1):
            se1_dset[jj,:,:] = se1_dsetONE
    else:
        se1_dset = se1_dset[0:No_experiments[index]+1,:,:]
    red1_dset = red1_dset[0:No_experiments[index]+1,:,:,:]
    blue = blue[0:No_experiments[index]+1,:,:,:]
    ...
    blue = blue/1.0e3
    red1_dset = np.array(red1_dset + blue, dtype=np.float32)    
    del blue
    gc.collect()
    
    mycode = 'ZZZZZZALLALLexptsdecayEr40CSA = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('ZZZZZZALLALLexptsdecayEr40CSA', data = red1_dset)
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    if index == 7000:  ### 7 was a problem for red, added a thousand
        ...
        init_guess = [np.average(gmmse_red1_darkse_dset[start_of_transient,:,:])/darkse_pct,0.1, np.average(gmmse_red1_darkse_dset[last_pt_offset,:,:])/darkse_pct, np.average(gmmse_red1_darkse_dset[start_of_transient+50,:,:])/darkse_pct, 0.05] #e init was 0.5
    else:
        init_guess = [np.average(gmmse_red1_darkse_dset[start_of_transient,:,:])/darkse_pct, 1.0, np.average(gmmse_red1_darkse_dset[last_pt_offset,:,:])/darkse_pct, np.average(gmmse_red1_darkse_dset[start_of_transient+50,:,:])/darkse_pct, 0.01] #e init was 0.5
...
#        np.savez('ZZZZZZEr60' + str(index), data = np.average(gmmse_red1_darkse_dset_for_4D/darkse_pct, axis=(1,2,3)))
    ...
    mycode = 'ZZZZZZALLdecayEr40CSA = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('ZZZZZZALLdecayEr40CSA', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
