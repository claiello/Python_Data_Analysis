# -*- coding: utf-8 -*-
"""
Created on Wed Feb  8 22:27:35 2017
...
class AnchoredScaleBar(AnchoredOffsetbox):
    def __init__(self, transform, size=0, label=None, horizontal = True, style = 'dark', loc=4,
        ...
        if my_fontsize is None:
            bars = packer(children=[bars, TextArea(label, dict(color = textcol), minimumdescent=False)],
                       align="center", pad=0, sep=sep)
        else:
            bars = packer(children=[bars, TextArea(label, dict(color = textcol, size = my_fontsize), minimumdescent=False)],
                       align="center", pad=0, sep=sep)
