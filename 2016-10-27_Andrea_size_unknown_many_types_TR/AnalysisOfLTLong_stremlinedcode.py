...
###############################################################################
###############################################################################
Time_bin = [40,1000,1000,1000,40]#in ns; 1/clock of 25MHz 
nominal_time_on = [1.8,3,3,5,1.8] #time during which e-beam nominally on, in mus

Pixel_size = [3.4, 3.5, 3.5, 3.5, 3.5] #nm
Ps = Pixel_size #pixel size in nm, the numbers above with round nm precision
No_exp = [1,1,1,1,1]
No_experiments = [1,1,1,1,1] #doin this bc all experiments were not saved, only the avg
...
for index in listofindex:
    ...
    tb = [40,1000,1000,1000,40]
    titulo = description + ' (10kV, 30$\mu$m aperture, ' + str(tb[index]) + 'ns time bins, ' + str(Ps)+ 'nm pixels, red/blue: $>/<$593nm)'
...
#        hlpse[hlpse == 1.0] = 0.0 #consider all light
    im = plt.imshow(se['data']*hlpse,cmap=cm.Greys) #or 'OrRd'
    sbar = sb.AnchoredScaleBar(ax1.transData, length_scalebar_in_pixels, scalebar_legend, style = 'bright', loc = 4)
    ax1.add_artist(sbar)
    plt.axis('off')
    del hlpse
    ...
    im = plt.imshow(np.nanmean(blue['data'],axis = (0)),cmap=cm.Blues) #or 'OrRd'
    ...
    imb = plt.imshow(np.nanmean(red['data'],axis = (0)),cmap=cm.Reds) #or 'OrRd'
    sbar = sb.AnchoredScaleBar(ax1.transData, length_scalebar_in_pixels, scalebar_legend, style = 'dark', loc = 4)
    ax1.add_artist(sbar)
    plt.axis('off')
...
#    if (index == 0) or (index == 1)or (index == 2):
#        hlp[np.isnan(hlp)] = 1.0 #consider all light
...
#    if (index == 0) or  (index == 1)or  (index == 2):
#        hlpd[hlpd == 1.0] = 0.0 #consider all light
   
    plt.plot(np.arange(0,red['data'].shape[0])*Time_bin[index]/1e3,np.nanmean(red['data']*hlp,axis = (1,2)),c='r',lw=2) #in mus, in MHz
    plt.plot(np.arange(0,red['data'].shape[0])*Time_bin[index]/1e3,np.nanmean(red['data']*hlpd,axis = (1,2)),c='DarkRed',lw=2) #in mus, in MHz
    plt.plot(np.arange(0,red['data'].shape[0])*Time_bin[index]/1e3,np.nanmean(blue['data']*hlp,axis = (1,2)),c='b',lw=2) #in mus, in MHz
    plt.plot(np.arange(0,red['data'].shape[0])*Time_bin[index]/1e3,np.nanmean(blue['data']*hlpd,axis = (1,2)),c='DarkBlue',lw=2) #in mus, in MHz
    inn = [0.2+0.3,1.0+0.3,1.0+0.3,1.0+0.3,0.2+0.3]
    enn = [ 0.2+1.8+0.3,1.0+3.0+0.3,1.0+3.0+0.3,1.0+5.0+0.3,0.2+1.8+0.3]
    ax1.axvspan(inn[index],enn[index], alpha=0.25, color='yellow')
    unit = 'kHz'
    plt.ylabel("Average luminescence \n of each time bin, per signal pixel (" + unit + ")",fontsize=fsizepl)
    vixi1 = [1.8,3,3,5,1.8]
    vixi2 = [4,96,96,94,4]
    plt.xlabel("Behaviour of e-beam during each experiment: \n " + str(vixi1[index]) + "-ON + "  + str(vixi2[index]) + "-OFF ($\mu$s)",fontsize=fsizepl)
    ...
    datared = red['data'] #np.average(red['data'], axis = (0))
    datablue = blue['data'] #np.average(blue['data'], axis = (0))
    
    if True is False:
        pass
    else:
        initbin = [5+45+9,1+3+1,1+3+1,1+5+1,5+45+9] #init bin for decay
        #effectively added: 2mus, 800ns, 600ns, 450ns
        backgdinit = [5,1,1,1,5]
        ### 700ns /40ns = 7. ....
        datared_init = datared[0:backgdinit[index],:,:]
        datared = datared[initbin[index]:,:,:]
        datablue_init = datablue[0:backgdinit[index],:,:]
    ...
    #hlpred = np.nanmean(red['data'][initbin[index]:,:,:]*hlp,axis = (1,2))
    #hlpblue = np.nanmean(blue['data'][initbin[index]:,:,:]*hlp,axis = (1,2))
    
    #error_arrayr = np.nanstd(hlpred,axis = 0)
    #error_arrayr[error_arrayr < 0.05 * np.max(hlpred)]  = 0.05 * np.max(hlpred) #arbitrary 5% of max  
    
    cut_longa = [6,6,6,6,6]
    cut_shorta = [3,3,3,3,3]
    
    init_guess = calc_double_fit(np.arange(0,datared.shape[0])*Time_bin[index]*1e-9*fastfactor,np.nanmean(datared*hlp,axis=(1,2)),dt= Time_bin[index]*1e-9*fastfactor,cut_long=cut_longa[index],cut_short=cut_shorta[index])
    init_guessb = calc_double_fit(np.arange(0,datablue.shape[0])*Time_bin[index]*1e-9*fastfactor,np.nanmean(datablue*hlp,axis=(1,2)),dt= Time_bin[index]*1e-9*fastfactor,cut_long=cut_longa[index],cut_short=cut_shorta[index])
    
    #replace c with cinit
    init_guess[2] = cinit 
    init_guessb[2] = cinitb
    
    b,e,be,ee,b2,e2,be2,ee2 = calcdecay_subplot_nan(datared*hlp, time_detail= Time_bin[index]*1e-9*fastfactor,titulo='',single=False,other_dset1=None,other_dset2=datablue*hlp,init_guess=init_guess,init_guess2=init_guessb,unit='kHz',error_array=None) #,error_array=error_arrayr, error_array2=error_arrayb)    
...
#    ee_array_blue[index] = ee2  
    ...
    xm = [4.0, 96,96,94, 4.0] 
    plt.xlim(xmax=xm[index])
    
    # Extra plots        
    aaa = datared*hlp
    xx_array = np.arange(0,aaa.shape[0])*Time_bin[index]*1e-9*fastfactor
    ...
    nominal_time_on = [1.8,3,3,5,1.8 ]
    
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLred[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2)),c='r', linestyle='None', marker='o',markersize=4) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLred[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2))*np.ones(1),c='r',  linewidth= lw) #in mus, in MHz
    #background intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLred[backgdinit[index]:initbin[index],:,:]*hlpd,axis=(0,1,2)),c='DarkRed', label='',linestyle='None', marker='o',markersize=3) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLred[backgdinit[index]:initbin[index],:,:]*hlpd,axis=(0,1,2))*np.ones(1),c='DarkRed',  linewidth= lw) #in mus, in MHz
    #init red intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLred[0:backgdinit[index],:,:]*hlp,axis=(0,1,2)),c='r',linestyle='None', marker='o',markersize=3) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLred[0:backgdinit[index],:,:]*hlp,axis=(0,1,2))*np.ones(1),'--',c='r',  linewidth= lw) #in mus, in MHz


    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLblue[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2)),c='b',linestyle='None', marker='o',markersize=4) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLblue[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2))*np.ones(1),c='b',  linewidth= lw) #in mus, in MHz
    #background intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLblue[backgdinit[index]:initbin[index],:,:]*hlpd,axis=(0,1,2)),c='DarkBlue', label='',linestyle='None', marker='o',markersize=3) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLblue[backgdinit[index]:initbin[index],:,:]*hlpd,axis=(0,1,2))*np.ones(1),c='DarkBlue',  linewidth= lw) #in mus, in MHz
    #init red intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLblue[0:backgdinit[index],:,:]*hlp,axis=(0,1,2)),c='b', linestyle='None', marker='o',markersize=3) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLblue[0:backgdinit[index],:,:]*hlp,axis=(0,1,2))*np.ones(1),'--',c='b',  linewidth= lw) #in mus, in MHz

    aa = np.nanmean(dataALLblue[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2))/np.nanmean(dataALLred[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2))
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,[np.nanmean(dataALLblue[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2))/np.nanmean(dataALLred[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2))],c='k', label='blue/red = ' + str("{0:.2f}".format(aa)),linestyle='None', marker='s',markersize=6) #in mus, in MHz
    plt.legend(loc='best')
    ...
    plt.xlim([nominal_time_on[index] - 1.0,nominal_time_on[index]*No_experiments[index]*fastfactor +1.0])
