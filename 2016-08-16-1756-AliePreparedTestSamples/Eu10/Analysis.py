...
### data
name = ['2016-08-16-1858_ImageSequence_Eu10_36.000kX_3.000kV_30mu_2.hdf5']
name_str = ['Eu10']
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    ###FIG4
    fig_no = '-3plots'
    start_of_transient = 83- cut_at_beginning  #time-bin 75 + 300ns/40ns = 75 + ~8 = 83
...
plt.plot(np.arange(0,71)*Time_bin/1e3,np.average(red['datared'],axis=(1,2)),'r',lw=3) #in mus, in MHz
plt.plot(np.arange(0,71)*Time_bin/1e3,np.average(blue['datablue'],axis=(1,2)),'b',lw=3)
    #else:
...
##SECOND COLUMN
name = ['2016-08-16-1904_ImageSequence_Eu10_36.000kX_3.000kV_30mu_3']
namefile = ['CheckBleach']
...
ax1 = plt.subplot2grid((2,2), (1, 0), colspan=1)
yap = np.load('ZZZCheckBleach.npz') #yap['datared'], yap['datablue']
#ax1.set_title('(Blue) YAP decay with e-beam',fontsize=fsizepl)
nominal_time_on = 2#in microsec
no_points = 20
plt.plot(np.arange(1,no_points+1)*nominal_time_on,np.average(yap['datablue'],axis=(1,2)),c='b',lw=3)
plt.plot(np.arange(1,no_points+1)*nominal_time_on,np.average(yap['datared'],axis=(1,2)),c='r',lw=3)
#plt.plot(np.arange(1,no_points+1)*nominal_time_on,np.average(yap['datablue']/1.0e3,axis=(0,1,2))*np.ones([no_points]),'k')
major_ticks = [10,20,30,40]
ax1.set_xticks(major_ticks) 
plt.xlim([2,40])
plt.ylabel('CL counts, per pixel (kHz)',fontsize=20)
#plt.ylim([0.42,0.48])
plt.xlabel('Cumulative e-beam exposure ($\mu$s)',fontsize=20)
