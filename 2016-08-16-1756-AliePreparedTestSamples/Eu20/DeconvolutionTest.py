...
zno = np.load('ZZZZnO3.npz') 
blue2 = np.average(zno['datablue'],axis=(0)) #kHz
initdecay = 83
impulse_response = np.average(blue2[initdecay:,:,:],axis=(1,2))

sred = np.average(red['datared'],axis=(1,2))
sblue = np.average(blue['datablue'],axis=(1,2))

recoveredRed, remainderRed = signal.deconvolve(sred[4:], impulse_response)
recoveredBlue, remainderBlue = signal.deconvolve(sblue[4:], impulse_response)
...
b,e,be,ee = calcdecay_subplot3(recoveredRed, time_detail= Time_bin*1e-9*fastfactor,titulo='Cathodoluminescence rate decay, bi-exponential fit, \n ' + titulo ,single=False,other_dset2=recoveredBlue ,other_dset1=None,init_guess=init_guess,unit='kHz',init_guess2=init_guess2)    
plt.xlim([0,2])
major_ticks0 = [1,2]
plt.ylabel("Average luminescence \n of each time bin, per pixel (" + unit + ")",fontsize=20)
