...
if do_plottau:
    ...
    def plotinho(ax0, dset,my_color,ax0b,my_edgecolor, my_facecolor,index,ct,linee):   
        ...
        ax0.plot(moving_average(np.arange(1,dset.shape[1]+1),movav),moving_average(unumpy.nominal_values(dset[index,:]),movav),color=my_color,ls='--',lw=linee)
       
        if True: #(ct == 5):
            ax0.fill_between(moving_average(np.arange(1,dset.shape[1]+1),movav),
                                                 moving_average(unumpy.nominal_values(dset[index,:]),movav)-moving_average(unumpy.std_devs(dset[index,:]),movav),
                                                 moving_average(unumpy.nominal_values(dset[index,:]),movav)+moving_average(unumpy.std_devs(dset[index,:]),movav),
        ...
        ax0.plot(moving_average(np.arange(1,dset.shape[1]+1),movav),moving_average(np.arange(1,dset.shape[1]+1)/2,movav),color='k',ls='--',lw=linee)        
        
        ax0b.plot(moving_average(np.arange(2,dset.shape[1]+1),movav),moving_average(np.diff(unumpy.nominal_values(dset[index,:])),movav),color=my_color,ls='--',lw=linee)
      
        ax0b.axhline(y=0.5, xmin=0, xmax=130, lw=2, color = 'k', ls='--')
        ax0b.set_yticks([0.25,0.5])#,200])#
        ax0b.set_ylim([0,0.55])#
        
        my_x= moving_average(np.arange(1,dset.shape[1]+1),movav)
    ...
    print('bef loading')
    C = pickle.load( open( "CEachAvg.p", "rb" ) ) #pickle.load( open( "d4.p", "rb" ) ) #4#
    ...
    print('here') 
    for kk in np.arange(0,taured.shape[0]):
        plotinho(ax00, taured[kk,:,:],'r',ax00b ,my_edgecolor='#ff3232', my_facecolor='#ff6666')
        plotinho(ax100, taublue[kk,:,:],'g',ax100b,my_edgecolor='#74C365', my_facecolor='#74C365')
    ...
    for kk in np.arange(0,taured.shape[0]):
        plotinho(ax00, taured[kk,:,:],'DarkRed',ax00b ,my_edgecolor='#801515', my_facecolor='#801515')
        plotinho(ax100, taublue[kk,:,:],'#003100',ax100b,my_edgecolor='#003D1B', my_facecolor='#003D1B')
    
    plt.tight_layout() 
    #plt.show()  
    print(counter)
    multipage_longer('IsTauSingleEachAvg.pdf',dpi=80)
