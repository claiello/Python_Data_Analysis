...
if do_plottau:
    ...
    print('bef loading')
    C = pickle.load( open( "AEachAvg.p", "rb" ) ) #pickle.load( open( "d4.p", "rb" ) ) #4#
    ...
    for jj in non_cut_k[0:1]: #::20]:
        
        print(counter)
        
        for kk in np.arange(0,taured.shape[0]):
            plotinho(ax00, taured[kk,:,:],'r',ax00b ,my_edgecolor='#ff3232', my_facecolor='#ff6666',index=jj,ct = counter,linee=linew[kk])
            plotinho(ax100, taublue[kk,:,:],'g',ax100b,my_edgecolor='#74C365', my_facecolor='#74C365',index=jj,ct = counter,linee=linew[kk])
    ...
    #plt.show()  
    print(counter)
    multipage_longer('IsTauSingleLARGEEachAvg.pdf',dpi=80)
