...
if do_plottau:
    ...
    ax00b.set_ylabel('Norm. standard deviation \n' + r'$\sigma_{\tau}/\tau$ ($\%$)',fontsize=fsizepl)
    ax00b.yaxis.set_label_position("right")
    ax100b.set_ylabel('Norm. standard deviation \n' + r'$\sigma_{\tau}/\tau$ ($\%$)',fontsize=fsizepl)
    ...
    def plotinho(ax0, dset,my_color,ax0b,my_edgecolor, my_facecolor,index,ct,linee):   
        
        movav = 100
        
        for kk in np.arange(0,dset.shape[0]):
            ax0.plot(moving_average(np.arange(1,dset.shape[2]+1),movav),moving_average(unumpy.nominal_values(dset[kk,index,:]),movav),color=my_color,ls='--',lw=1)
       
            if True: #(ct == 5):
                ax0.fill_between(moving_average(np.arange(1,dset.shape[2]+1),movav),
                                                 moving_average(unumpy.nominal_values(dset[kk,index,:]),movav)-moving_average(unumpy.std_devs(dset[kk,index,:]),movav),
                                                 moving_average(unumpy.nominal_values(dset[kk,index,:]),movav)+moving_average(unumpy.std_devs(dset[kk,index,:]),movav),
        ...
        #if linee == 1:
        ax0.plot(moving_average(np.arange(1,dset.shape[2]+1),movav),moving_average(np.arange(1,dset.shape[2]+1)/2,movav),color='k',ls='--',lw=2) 
        
        
        #spread = np.amax(unumpy.nominal_values(dset[:,index,:]), axis = 0) - np.amin(unumpy.nominal_values(dset[:,index,:]), axis = 0) 
        spread = 100*np.std(unumpy.nominal_values(dset[:,index,:]), axis = 0)/np.average(unumpy.nominal_values(dset[:,index,:]), axis = 0)
        #spread = 100*np.std(dset[:,index,:], axis = 0)/np.average(dset[:,index,:], axis = 0) #does not work


        ax0b.plot(moving_average(np.arange(1,dset.shape[2]+1),movav),moving_average(unumpy.nominal_values(spread),movav),color=my_color,ls='--',lw=2)
        ax0b.fill_between(moving_average(np.arange(1,dset.shape[2]+1),movav),
                                                 moving_average(unumpy.nominal_values(spread),movav)-moving_average(unumpy.std_devs(spread),movav),
                                                 moving_average(unumpy.nominal_values(spread),movav)+moving_average(unumpy.std_devs(spread),movav),
                                                 edgecolor=my_edgecolor,
                                                 facecolor=my_facecolor,
                                                 alpha=0.5,
       ...
       # ax0b.set_ylim([0,0.55])#
        ...
        my_x= moving_average(np.arange(1,dset.shape[2]+1),movav)
        ...
        ax0b.set_ylim([0,14])
        ax0b.set_yticks([2.5, 5, 7.5, 10, 12.5])
        
    print('bef loading')
    C = pickle.load( open( "BEachAvg.p", "rb" ) ) #pickle.load( open( "d4.p", "rb" ) ) #4#
    ...
    taured = unumpy.uarray(C['taured'], C['tauredstd'])
    taublue =  unumpy.uarray(C['taublue'], C['taubluestd'])
    
    counter = 0
    linew = [1,2,3,4,5]
    
    for jj in non_cut_k[8:9]: #::20]:
        
        print(counter)
        
        plotinho(ax00, taured,'r',ax00b ,my_edgecolor='#ff3232', my_facecolor='#ff6666',index=jj,ct = counter,linee=1)
        plotinho(ax100, taublue,'g',ax100b,my_edgecolor='#74C365', my_facecolor='#74C365',index=jj,ct = counter,linee=1)
    ...
    #plt.show()  
    print(counter)
    multipage_longer('IsTauSingleMEDIUMEachAvg.pdf',dpi=80)
