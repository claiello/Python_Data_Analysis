...
if do_plottau:
    ...
    def plotinho(ax0, dset,my_color,ax0b,my_edgecolor, my_facecolor,index,ct):   
...
#                arrowprops=dict(facecolor='black', shrink=0.05))              
        ...
        ax0.plot(moving_average(np.arange(1,taured.shape[1]+1),movav),moving_average(unumpy.nominal_values(dset[index,:]),movav),color=my_color,ls='--',lw=2)
        ...
        ax0b.plot(moving_average(np.arange(2,taured.shape[1]+1),movav),moving_average(np.diff(unumpy.nominal_values(dset[index,:])),movav),color=my_color,ls='--',lw=2)
    ...
    print('bef loading')
    C = pickle.load( open( "B.p", "rb" ) ) #pickle.load( open( "d4.p", "rb" ) ) #4#
    ...
    print(intensr[non_cut_k].shape)
    
    print(np.mean(intensr[non_cut_k]))
    print(np.mean(intensb[non_cut_k]))
    
    print(np.mean(intensr[non_cut_k])/np.mean(areas[non_cut_k])*2.5*2.5)
    print(np.mean(intensb[non_cut_k])/np.mean(areas[non_cut_k])*2.5*2.5)
    klklklk    
    ...
    for jj in non_cut_k:
        ...
        plotinho(ax00, taured,'r',ax00b ,my_edgecolor='#ff3232', my_facecolor='#ff6666',index=jj,ct = counter)
        plotinho(ax100, taublue,'g',ax100b,my_edgecolor='#74C365', my_facecolor='#74C365',index=jj,ct = counter)
    ...
    #plt.show()  
    print(counter)
    multipage_longer('IsTauSingleMEDIUM.pdf',dpi=80)
