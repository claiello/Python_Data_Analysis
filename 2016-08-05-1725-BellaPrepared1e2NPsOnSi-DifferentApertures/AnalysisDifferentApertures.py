...
#30mum ->2016-08-05-1809_ImageSequence_Er2-30mum_4.042kX_2.000kV_30mu_1
#60mum -> 2016-08-05-1851_ImageSequence_Er2-60mum_4.000kX_2.000kV_60mu_1
#120mum -> 2016-08-05-1938_ImageSequence_Er2-120mum_4.000kX_2.000kV_12mu_1
#No_pixels = 250
...
### data
name = ['30mum.hdf5', '60mum.hdf5','120mum.hdf5',]
name_str = ['Er2_30mum','Er2_60mum','Er2_120mum']
na = ['2','2','2']
pmt = ['PMT red','PMT red','PMT red']
channel = ['1','1','1']
ap = ['30','60','120']
Pixel_size = [2.7623802e-08,2.7912499e-08,2.7912499e-08]
Ps = [28,28,28] #pixel size in nm, the numbers above with round nm precision

#no experiments to consider
No_experiments = [50,50,50] #only "up" in luminescence curve
...
#for index in np.arange(12,12):
    ...
    titulo =  'Er ' +  na[index] + '$\%$ core-shell aggregates (4kX, 2kV, ' +  ap[index] + '$\mu$m, 40ns time bins, ' + str(Ps[index]) + 'nm pixels)' 
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    if do_gmmse_dset:
        ...
        gmmse_red1_brightse_dset = red1_dset_cut #or could give 0 vector
        darkse_pct = 1.0
        brightse_pct = 0.0
    ...
    mycode = 'ZZZZZZEr2' + str(index) + '= tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('ZZZZZZEr2' + str(index), data = np.average(gmmse_red1_darkse_dset_for_4D/darkse_pct, axis=(1,2,3)))
...
#plot_expt_by_expt_behaviour_Er60_1over(titulo, data30['data'], data60['data'], data120['data'], Time_bin, nominal_time_on,fastfactor,'r',[25,50,75,100],no_points,aper, current)   
