...
nametr = ['2016-11-07-1459_ImageSequence__17.192kX_10.000kV_30mu_2',
          '2016-11-07-1510_ImageSequence__17.192kX_10.000kV_30mu_3',
          '2016-11-07-1522_ImageSequence__17.192kX_10.000kV_30mu_4']
namese = ['2016-11-07-1447_ImageSequence__17.192kX_10.000kV_30mu_1',
         '2016-11-07-1447_ImageSequence__17.192kX_10.000kV_30mu_1',
         '2016-11-07-1447_ImageSequence__17.192kX_10.000kV_30mu_1'] 
...
#    
    ...
    if index == 0: #needs to be reworked out
       init_guess = [np.nanmean(hlp_red[0,:,:]), 5.0, np.nanmean(hlp_red[last_pt_offset,:,:]), np.nanmean(hlp_red[10:50,:,:]), 20.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 5.0,np.nanmean(hlp_blue[last_pt_offset,:,:]), np.nanmean(hlp_blue[10:50,:,:]), 20.0 ] #BLUE
    ...
    if index == 2:
        init_guess = [np.nanmean(hlp_red[0,:,:]), 5.0, np.nanmean(hlp_red[last_pt_offset,:,:]), np.nanmean(hlp_red[10:50,:,:]), 21.0 ] #RED
        init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 5.0,np.nanmean(hlp_blue[last_pt_offset,:,:]), np.nanmean(hlp_blue[10:50,:,:]), 23.0 ] #BLUE
    ...
    b,e,be,ee,b2,e2,be2,ee2 = calcdecay_subplot_nan(datared*hlp, time_detail= Time_bin*1e-9*fastfactor,titulo='Cathodoluminescence rate decay, bi-exponential fit, \n ' + titulo ,single=True,other_dset2=datablue*hlp ,other_dset1=None,init_guess=init_guess,unit='kHz',init_guess2=init_guess2) #,error_array=error_arrayr, error_array2=error_arrayb)    
...
#######################ACHTUNG!!!!!!!
#there is a mistake in il_data[0], which was read as 40mV!!!!! replacing it by another value just to make it plottable:
il_data[0] = 1.05*1e-3
