########################### VEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEERY BAD
#### BAD< COMMENTED FILE, DONT USE THIS AS EXAMPLE FOR NEXT FILE
#USE ONE ABOVE
...
nominal_time_on = 90.0 #time during which e-beam nominally on, in mus
totalpoints = 1600 #total number of time-resolved points
### data
...
nametr = ['2016-12-12-1420_ImageSequence__100.000kX_10.000kV_30mu_2',
          '2016-12-12-1458_ImageSequence_0pt5V_100.000kX_10.000kV_30mu_1',
          '2016-12-12-1534_ImageSequence_0pt5V_101.045kX_10.000kV_30mu_2']
...
#for index in np.arange(0,3):
   ...
   # red0_dset  = file2['/data/Counter channel 1 : PMT red/PMT red time-resolved/data']#10 Scany points X10 frames x 150 tr pts x250 x 250 pixels
    red0_dset  = file2['/data/Counter channel 2 : PMT blue/PMT blue time-resolved/data']#10 Scany points X10 frames x 150 tr pts x250 x 250 pixels
    ...
    ###############################################################################
    #independently
    #se1_dset_reg = reg_images(se1_dset2)
    ...
    se1_dset_reg = se1_dset2
    red1_dset_reg = red1_dset
    red1_dset_reg_all = red1_dset
...
#for index in listofindex:
    ...
    titulo = 'Region ' + let[index] + '(10kV, 30$\mu$m aperture, 1$\mu$s time bins, ' + str(Ps)+ 'nm pixels, blue/red: $</>$ 593nm)'
    ...
    #else:
    plt.suptitle("Registration and segmentation (model: 2-GMM) of cathodoluminescence signal using SE channel, !INIT BIN 102 not 100!, \n" + titulo,fontsize=fsizetit)
   
    gc.collect()
    
    ax1 = plt.subplot2grid((2,3), (0, 0), colspan=1)
    ax1.set_title('SE channel',fontsize=fsizepl) #as per accompanying txt files
    plt.imshow(se['data'].reshape(500,500),cmap=cm.Greys_r)
    ...
    hlp = segmm['bright'].reshape(500,500)
...
#        hlp[~np.isnan(hlp)] = 1.0
    ...
    plt.plot(np.arange(0,1600)*Time_bin/1e3,np.nanmean(red['data']*hlp,axis = (0,2,3)),c='r',label='Red photons ($>$ 593nm)',lw=3) #in mus, in MHz
    plt.plot(np.arange(0,1600)*Time_bin/1e3,np.nanmean(blue['data']*hlp,axis = (0,2,3)),c='b',label='Blue photons ($<$ 593nm)',lw=3) #in mus, in MHz
    ...
    if True is False:
        ...
        initbin = 102#95#102 #### ACHTUNG!!!!!! WAS 95 ORIGINALLY!!!!!!!!!!!!!!!!!
...
ax1.set_xlabel('Voltage on heater (V)')
x_vec = [0, 0.5,1] #(6,10)
