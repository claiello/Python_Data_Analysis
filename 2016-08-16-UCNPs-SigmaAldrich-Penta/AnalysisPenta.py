...
#        np.savez('ZZZZZZEr60' + str(index), data = np.average(gmmse_red1_darkse_dset_for_4D/darkse_pct, axis=(1,2,3)))
    ...
    mycode = 'tkvZZZRedDecay = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('tkvZZZRedDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
...
local_maxi = peak_local_max(distance, num_peaks = 9, indices = False, footprint=np.ones((15,15)),labels=image) #footprint = min dist between maxima to find #footprint was 25,25
...
plt.plot(np.arange(1,50)*nominal_time_on*fastfactor,np.average(dataALLblue,axis=(1,2,3)),c='b', label='From blue photons ($<$ 593nm)',linestyle='None', marker='o',markersize=4) #in mus, in MHz
plt.plot(np.arange(1,50)*nominal_time_on*fastfactor,np.average(dataALLred,axis=(1,2,3)),c='r', label='From red photons ($>$ 593nm)',linestyle='None', marker='o',markersize=4) #in mus, in MHz
