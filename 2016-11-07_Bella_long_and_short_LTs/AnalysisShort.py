...
nominal_time_on = 1.6#time during which e-beam nominally on, in mus
totalpoints = 400 #total number of time-resolved points
### data
...
nametr = ['2016-11-07-1217_ImageSequence_Bella10_4.953kX_10.000kV_30mu_4',
          '2016-11-07-1246_ImageSequence_Bella20_12.239kX_10.000kV_30mu_8',
          '2016-11-07-1306_ImageSequence_Bella40_11.987kX_10.000kV_30mu_4',
          '2016-11-07-1322_ImageSequence_Bella40_24.000kX_10.000kV_30mu_7',
          '2016-11-07-1337_ImageSequence_Bella40_48.000kX_10.000kV_30mu_10',
          '2016-11-07-1357_ImageSequence_Bella40_12.380kX_10.000kV_30mu_15']
          
#Corresponding SEs: 12:18, 12:47, 1:07, 1:23,1.38, 1:58
#SE NOT visible in the TR files. Not doing SE segmentation.
...
##    
    ...
    exec(mycode)
    np.savez(str(let[index]) +'SRedbright', data = red1_dset_cut)
    
    mycode = str(let[index]) + 'SBluebright = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) +'SBluebright', data = blue1_dset_cut)
...
#    
    ...
    if index == 0: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:]), 0.1, np.nanmean(hlp_red[last_pt_offset,:,:]), np.nanmean(hlp_red[10:50,:,:]), 1.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 0.1,np.nanmean(hlp_blue[last_pt_offset,:,:]), np.nanmean(hlp_blue[10:50,:,:]), 1.0 ] #BLUE
    ...
    if index == 1:
        #init_guess = [8.27,0.04,np.nanmean(hlp_red[last_pt_offset,:,:]),0.03,2.98] #RED
        #init_guess2 = [10.58,0.03,np.nanmean(hlp_blue[last_pt_offset,:,:]),0.087,0.5] #BLUE
        init_guess = [np.nanmean(hlp_red[0,:,:]), 0.1, np.nanmean(hlp_red[last_pt_offset,:,:]), np.nanmean(hlp_red[10:50,:,:]), 0.9] #RED
        init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 0.1,np.nanmean(hlp_blue[last_pt_offset,:,:]), np.nanmean(hlp_blue[10:50,:,:]), 1.0 ] #BLUE
    ...
    if index == 5:
        #pass
        init_guess = [np.nanmean(hlp_red[0,:,:]), 0.1, np.nanmean(hlp_red[last_pt_offset,:,:]), np.nanmean(hlp_red[10:50,:,:]), 0.5 ] #RED
        init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 0.1,np.nanmean(hlp_blue[last_pt_offset,:,:]), np.nanmean(hlp_blue[10:50,:,:]), 5.0 ] #BLUE
...
#    error_arrayb[error_arrayb < 0.05 * np.max(hlpblue)]  = 0.05 * np.max(hlpblue)
    ...
    #ACHTUNG!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #Times calculated for Er20 RED were with Fixed C, otherwise error too large
    #replace those
    if index == 1:
        b_array_red[index] = 0.05
        e_array_red[index] = 2.975
        be_array_red[index] = 0.002  
        ee_array_red[index] = 6.457
    #All other times calculated with varying C and same init conditions
...
mycode = 'SRed_int_array = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SRed_int_array', data = red_int_array)

mycode = 'SBlue_int_array = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SBlue_int_array', data = blue_int_array)

mycode = 'SB_array_red = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SB_array_red', data = b_array_red)

mycode ='SBe_array_red = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SBe_array_red', data = be_array_red)

mycode = 'SE_array_red = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SE_array_red', data = e_array_red)

mycode = 'SEe_array_red = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SEe_array_red', data = ee_array_red)

mycode = 'SB_array_blue = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SB_array_blue', data = b_array_blue)

mycode ='SBe_array_blue = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SBe_array_blue', data = be_array_blue)

mycode = 'SE_array_blue = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SE_array_blue', data = e_array_blue)

mycode = 'SEe_array_blue = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez('SEe_array_blue', data = ee_array_blue)

#####FIG WITH SUMMARY   
fastfactor = 1

Red_int_array = np.load('SRed_int_array.npz') 
red_int_array = Red_int_array['data']
Blue_int_array = np.load('SBlue_int_array.npz') 
blue_int_array = Blue_int_array['data']

B_array_red= np.load('SB_array_red.npz')
b_array_red = B_array_red['data']  
Be_array_red = np.load('SBe_array_red.npz')
be_array_red = Be_array_red['data'] 
E_array_red = np.load('SE_array_red.npz')
e_array_red = E_array_red['data']   
Ee_array_red = np.load('SEe_array_red.npz')
ee_array_red = Ee_array_red['data']   

B_array_blue= np.load('SB_array_blue.npz')
b_array_blue = B_array_blue['data']  
Be_array_blue = np.load('SBe_array_blue.npz')
be_array_blue = Be_array_blue['data'] 
E_array_blue = np.load('SE_array_blue.npz')
e_array_blue = E_array_blue['data']   
Ee_array_blue = np.load('SEe_array_blue.npz')
...
plt.rc('font', family='serif')
plt.rc('font', serif='Palatino')    
fig41.suptitle('Lifetimes fitted after 10$\mu$s, NaYF$_4$ CS from Bella \n (10kV, 30$\mu$m aperture, 12kX, 40Ns time bins)',fontsize=fsizetit)     
...
multipage_longer('SZZZZZSummary.pdf',dpi=80)
