...
Pixel_size =  np.ones([len(name)])*9.9e-8  #[5.6e-08, 5.6e-08, 5.6e-08, 5.6e-08, 5.6e-08, 5.8e-08,  5.9e-08] #as given by accompanying text file
Ps = np.ones([len(name)])*99 #[56,56,56,56,56,58,59]
lag = np.ones([len(name)])*99 #[500, 50,50,50,50,50,50]
frames = np.ones([len(name)])*99 #[20, 50,50,50,100,100,100] 
...
index = 1
listofindices = np.arange(0,len(name))

for index in [18,19,20,21,22,23]: #listofindices:
    ...
    #plt.close("all")
    title =  'F888 with ZnO:Ga with pmma' #'Grana + IL on ZnO:Ga (4kV, 30 $\mu$m, ' + str(Ps[index]) + 'nm pixels, ' + str(lag[index]) + '$\mu$s lag per pixel, ' + str(frames[index]) + 'expts., SE registered), \n' #+ obs[index] 
