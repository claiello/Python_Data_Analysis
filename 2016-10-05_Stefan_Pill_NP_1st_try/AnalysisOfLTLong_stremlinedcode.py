...
#for index in listofindex:
    ...
    length_scalebar = 10.0 #in nm 
    scalebar_legend = '10nm'
    length_scalebar_in_pixels = np.ceil(length_scalebar/(Pixel_size[index]))
    
    titulo = description[index] + ' (10kV, 30$\mu$m aperture, 100ns time bins, ' + str(Ps)+ 'nm pixels, red/blue: $>$715nm/$<$650nm)'
    ...
    ax1.axvspan(0.2+0.3,0.5+2.5+0.3, alpha=0.25, color='yellow')
    unit = 'kHz'
    ...
    if True is False:
        ...
        initbin = (83)-1 #init bin for decay
        backgdinit = 12
        ### 700ns /40ns = 7. ....
