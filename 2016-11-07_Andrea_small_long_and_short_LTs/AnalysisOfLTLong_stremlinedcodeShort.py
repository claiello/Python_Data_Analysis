...
for index in listofindex:
    ...
    titulo = description + ' (10kV, 30$\mu$m aperture, 40ns time bins, ' + str(Ps)+ 'nm pixels, blue/red: $</>$ 593nm)'
    ...
    if (index == 0) or (index == 1):
        ax1.axvspan(0.04+0.3,0.04+0.96+0.3, alpha=0.25, color='yellow')
    ...
    if True is False:
        ...
        if (index == 0) or (index == 1):
            initbin = (1+24+9)#(102)-1 #init bin for decay
            backgdinit = 1
        if (index == 2) or (index == 3):  
            initbin = (10+40+9)#(102)-1 #init bin for decay
            backgdinit = 10
        ### 700ns /40ns = 7. ....
