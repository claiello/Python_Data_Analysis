...
prefixend = 'fixCsametaudouble_fitwitherror'
vary_offset = False


#prefixend = 'varCsametaudouble_fitwitherror'
#vary_offset = True

#prefixend = 'fixCsametaudouble_nodiv'
#prefixend = 'fixCprevtaudouble_nodiv'
#refixend = 'varCsametaudouble_nodiv'
...
init_tau_longa = 50.0 #10.0
init_tau_shorta = 5.0 #1.0

#index = 4
#if index is 4:
for index in [0,1,2,3,4]: #listofindex:
    
    print(index)
    
    print('bef loading')
    redd = np.load('varCsametaudouble' + let[index] + 'RED1D.npz',mmap_mode='r') #any prefix will do
    blued = np.load('varCsametaudouble' + let[index] + 'BLUE1D.npz',mmap_mode='r') #any prefix will do
    red = redd['data']
    blue = blued['data']
    ...
    init_guess = calc_double_fit(np.arange(0,red.shape[0])*Time_bin*1.0e-9*fastfactor,red,dt= Time_bin*1.0e-9*fastfactor,cut_long=cut_longa,cut_short=cut_shorta,init_tau_long = init_tau_longa, init_tau_short=init_tau_shorta)
    
    gc.collect()    
    
    init_guessb = calc_double_fit(np.arange(0,blue.shape[0])*Time_bin*1.0e-9*fastfactor,blue,dt= Time_bin*1.0e-9*fastfactor,cut_long=cut_longa,cut_short=cut_shorta,init_tau_long = init_tau_longa, init_tau_short=init_tau_shorta)
    
    
    back_init_red = np.load('Back_array_red.npz',mmap_mode='r') #any prefix will do
    back_init_blue = np.load('Back_array_blue.npz',mmap_mode='r') #any prefix will do
    init_guess[2] = back_init_red['data'][index]
    init_guessb[2] = back_init_blue['data'][index]
    
    std_red = np.load('Std_array_red.npz',mmap_mode='r') #any prefix will do
    std_blue = np.load('Std_array_blue.npz',mmap_mode='r') #any prefix will do
...
#    if (prefixend == 'fixCsametaudouble_fitwitherror') and (index == 1):
#         init_guessb[1] = (12.0 + 16.0)/2.0
#         init_guessb[4] = (108.0 + 117.0)/2.0
    if (prefixend == 'fixCsametaudouble_fitwitherror') and (index == 4):
         init_guessb[1] = 40.0
         init_guessb[4] = 127.0
    
    b,e,be,ee,b2,e2,be2,ee2,chisquared,chisquared2 = calcdecay_subplot_nan_1D_with_error(red, time_detail= Time_bin*1.0e-9*fastfactor,titulo='',single=False,other_dset1=None,other_dset2=blue,init_guess=init_guess,init_guess2=init_guessb,unit='kHz',error_array=None, vary_offset_desired = vary_offset, yerrinput = redstd, yerr2input = bluestd) #,error_array=error_arrayr, error_array2=error_arrayb)    
