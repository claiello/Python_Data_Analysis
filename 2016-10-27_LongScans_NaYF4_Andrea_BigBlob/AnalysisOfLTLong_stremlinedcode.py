...
###############################################################################
###############################################################################
Time_bin = [1000,200,100,50]#in ns; 1/clock of 25MHz 
nominal_time_on = 2.8 #time during which e-beam nominally on, in mus

Pixel_size = [3.5, 3.5, 3.5, 3.5] #nm
Ps = Pixel_size #pixel size in nm, the numbers above with round nm precision
No_exp = [10,10,10,10]
No_experiments = [1,1,1,1] #doin this bc all experiments were not saved, only the avg
...
#        hlpse[hlpse == 1.0] = 0.0 #consider all light
    ...
    ax1.set_title('CL data averaged over time \n and ' + str(No_exp[index]) +  ' experiments',fontsize=fsizepl)
...
#        hlpd[hlpd == 1.0] = 0.0 #consider all light
    ...
    inn = [1.0+0.3,1.0+0.3,1.0+0.3,1.0+0.3]
    enn = [1.0+0.3+5.0+1.0,1.0+0.3+5.0,1.0+0.3+5.0,1.0+0.3+5.0]
    ...
    if True is False:
        ...
        initbin = [(1+5+2)-1,(5+25+4)-1,(10+50+6)-1,(20+100+9)-1] #init bin for decay
        #effectively added: 2mus, 800ns, 600ns, 450ns
        backgdinit = [1,5+1,10+3,20+6]
        ### 700ns /40ns = 7. ....
...
#    ee_array_blue[index] = ee2  
    ...
    nominal_time_on = [5.0,5.0,5.0,5.0]*No_exp[index]
    
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLred[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2)),c='r', label='From photons $>$ 509nm',linestyle='None', marker='o',markersize=4) #in mus, in MHz
    ...
    #init red intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLred[0:backgdinit[index],:,:]*hlp,axis=(0,1,2)),c='r', label='',linestyle='None', marker='o',markersize=3) #in mus, in MHz
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLblue[backgdinit[index]:initbin[index],:,:]*hlp,axis=(0,1,2)),c='b', label='From photons $>$ 509nm',linestyle='None', marker='o',markersize=4) #in mus, in MHz
    ...
    #init red intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on[index]*fastfactor,np.nanmean(dataALLblue[0:backgdinit[index],:,:]*hlp,axis=(0,1,2)),c='b', label='',linestyle='None', marker='o',markersize=3) #in mus, in MHz
