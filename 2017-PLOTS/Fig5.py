...
if True:
    ...
    starrtred = 25
    starrtgreen = 15
    ax4.plot(np.arange(1+starrtred,taured.shape[1]+1),100.0*aa[starrtred:]/unumpy.nominal_values(taured[indicetonorm,starrtred:]),ls='--',color='r',lw=2)
    ax4.plot(np.arange(1+starrtgreen,taured.shape[1]+1),100.0*cc[starrtgreen:]/unumpy.nominal_values(taublue[indicetonorm,starrtgreen:]),ls='--',color='g',lw=2)
    ...
    ####FIND MINIMUM
    minslopered = np.min(100.0*aa/unumpy.nominal_values(taured[indicetonorm,:]))
    timeminslopered = find_nearest(100.0*aa/unumpy.nominal_values(taured[indicetonorm,:]),minslopered)
    minslopegreen = np.min(100.0*cc/unumpy.nominal_values(taublue[indicetonorm,:]))
    timeminslopegreen = find_nearest(100.0*cc/unumpy.nominal_values(taublue[indicetonorm,:]),minslopegreen)
...
#    ax4.vlines(timeminslopegreen,ymin=-1, ymax=minslopegreen,linestyle='dashed',color='g',lw=2,zorder=1)
    ...
    (xxx, eta_ratio) = get_sens((taublue)/(taured), xvec,np.arange(1,taured.shape[1]+1))
    ...
    startpt = 10
    ax0.semilogy(np.arange(1+startpt,taured.shape[1]+1),eta_visib[startpt:] ,color='c',ls='--',lw=2)
    ax0.semilogy(np.arange(1+startpt,taured.shape[1]+1),eta_ratio[startpt:] ,color='m',ls='--',lw=2)
