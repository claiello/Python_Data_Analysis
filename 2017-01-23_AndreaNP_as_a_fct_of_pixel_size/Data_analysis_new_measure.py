...
def plot_things(prefix, my_color):
    ...
    for index in listofindex: 
        ...
        ax1.semilogy( aux_vec[index,:], np.arange(0,len(red)),lw=len(listofindex)-index,color=my_color)
...
pickle.dump(save_data, open("../d2.p", "wb"))
...
#    ax10.set_title('Lw $\propto$ pixel size; this plot not yet making sense')
    ...
    ax20.plot(np.arange(0,aux_vec_red.shape[1]), aux_vec_cumu_red[index,:]-aux_vec_cumu_blue[index,:],lw=len(listofindex)-index,color='k',ls='-') #label='minus')
    ax200.plot(np.arange(0,aux_vec_red.shape[1]), aux_vec_cumu_red[index,:]/aux_vec_cumu_blue[index,:],lw=len(listofindex)-index,color='k',ls='-') #,label='ratio')
    ax2000.plot(np.arange(0,aux_vec_red.shape[1]), (aux_vec_cumu_red[index,:]-aux_vec_cumu_blue[index,:])/(aux_vec_cumu_red[index,:]+aux_vec_cumu_blue[index,:]),lw=len(listofindex)-index,color='k',ls='-')#,label='visi')
    ax20.set_xlabel('tau')
