...
#MIND THE PREFIX1111111111111111111111111111111111111111111111111111

prefix = 'fixedC' #'varCprevtau' #'varCprevtau'
...
Pixel_size = 2.5*np.ones(len(nametr))
pisize =Pixel_size
...
consider_whole_light = []# [0,1,2,3,4,5,6]
...
for index in  [4,5,6,7]: #listofindex:
    ...
    print('bef loading')
    se = np.load(str(let[index]) +'SEchannelCUT.npz',mmap_mode='r') 
    segmm = np.load(str(let[index]) +'SEchannelGMMCUT.npz',mmap_mode='r') 
    red = np.load(str(let[index]) +'RedbrightCUT.npz',mmap_mode='r') 
    blue = np.load(str(let[index]) +'BluebrightCUT.npz',mmap_mode='r') 
...
#==============================================================================
    ...
    im = plt.imshow(np.nanmean(blue['data'][:,backgdinit:initbin,:,:],axis = (0,1)),cmap=cm.Greens) #or 'OrRd'
    ...
    print(np.nanmean(blue['data'][:,backgdinit:initbin,:,:],axis = (0,1,2,3)))
    tickval = np.nanmean(blue['data'][:,backgdinit:initbin,:,:],axis = (0,1,2,3))
    ...
    ax1.set_title('CL data while e-beam on,\n averaged over 5 experiments',fontsize=fsizepl)
    imb = plt.imshow(np.nanmean(red['data'][:,backgdinit:initbin,:,:],axis = (0,1)),cmap=cm.Reds) #or 'OrRd'
    ...
    tickval =  np.nanmean(red['data'][:,backgdinit:initbin,:,:],axis = (0,1,2,3))
    ...
    plt.plot(np.arange(0,red['data'].shape[1])*Time_bin/1.0e3,np.nanmean(red['data']*hlp,axis = (0,2,3)),c='r',lw=2) #in mus, in MHz
    ...
    plt.plot(np.arange(0,red['data'].shape[1])*Time_bin/1.0e3,np.nanmean(blue['data']*hlp,axis = (0,2,3)),c='g',lw=2) #in mus, in MHz
    ...
    datared = np.nanmean(red['data'], axis = (0))
    datablue = np.nanmean(blue['data'], axis = (0))
...
#       pass
#    if index == 5:
#        init_guess[1] = b_array_red[4]
#        init_guess[4] = e_array_red[4]
#        init_guessb[1] = b_array_blue[4]
#        init_guessb[4] = e_array_blue[4]
#    if index == 6:
#        init_guess[1] = (b_array_red[5] + b_array_red[4])/2
#        init_guess[4] = (e_array_red[5] + e_array_red[4])/2
#        init_guessb[1] = (b_array_blue[5] + b_array_blue[4])/2
#        init_guessb[4] = (e_array_blue[5] + e_array_blue[4])/2
#    if index == 7:
#        init_guess[1] = (b_array_red[6] + b_array_red[5] + b_array_red[4])/3
#        init_guess[4] = (e_array_red[6] + e_array_red[5] + e_array_red[4])/3
#        init_guessb[1] = (b_array_blue[6] + b_array_blue[5] + b_array_blue[4])/3
#        init_guessb[4] = (e_array_blue[6] + e_array_blue[5] + e_array_blue[4])/3
#        
     #################################### THIS IS TO GIVE PREV TAU AS INIT
    print(init_guessb[1])
    print(init_guessb[4])
    ...
    plt.semilogy(xx_array/1.0e-6,np.nanmean(datared_init*hlp,axis=(0,1,2))*np.ones(len(xx_array)),'r--',lw=2,label='Mean CL from signal pixels, before e-beam')
    ...
    #Plot mean signal before e-beam on
    plt.semilogy(xx_array/1.0e-6,np.nanmean(datablue_init*hlp,axis=(0,1,2))*np.ones(len(xx_array)),'g--',lw=2,label='Mean CL from blue signal pixels, before e-beam')
    #Plot mean background
...
#        dataALLblue = blue['data'][0:3,:,:,:]
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue[:,backgdinit:initbin,:,:]*hlp,axis=(1,2,3)),c='g', linestyle='None', marker='o',markersize=4) #in mus, in MHz
    ...
    #init red intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue[:,0:backgdinit,:,:]*hlp,axis=(1,2,3)),c='g', linestyle='None', marker='o',markersize=3) #in mus, in MHz
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue[:,backgdinit:initbin,:,:]*hlp,axis=(1,2,3))/np.nanmean(dataALLred[:,backgdinit:initbin,:,:]*hlp,axis=(1,2,3)),c='k',linestyle='None', marker='s',markersize=3)# + str("{0:.2f}".format(aa[0])),linestyle='None', marker='s',markersize=6) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue[:,backgdinit:initbin,:,:]*hlp,axis=(0,1,2,3))/np.nanmean(dataALLred[:,backgdinit:initbin,:,:]*hlp,axis=(0,1,2,3))*np.ones(dataALLred.shape[0]),c='k',  linewidth= lw,label='green/red signal (a.u.)' ) #in mus, in MHz
...
mycode = prefix +'Red_std_arrayCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'Red_std_arrayCUT', data = red_std_array)

mycode = prefix +'Blue_std_arrayCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'Blue_std_arrayCUT', data = blue_std_array)

mycode = prefix + 'Red_int_arrayCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'Red_int_arrayCUT', data = red_int_array)

mycode = prefix +'Blue_int_arrayCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'Blue_int_arrayCUT', data = blue_int_array)
...
mycode =prefix + 'B_array_redCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'B_array_redCUT', data = b_array_red)

mycode =prefix +'Be_array_redCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'Be_array_redCUT', data = be_array_red)

mycode = prefix +'E_array_redCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'E_array_redCUT', data = e_array_red)

mycode = prefix +'Ee_array_redCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'Ee_array_redCUT', data = ee_array_red)

mycode = prefix +'B_array_blueCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'B_array_blueCUT', data = b_array_blue)

mycode =prefix +'Be_array_blueCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'Be_array_blueCUT', data = be_array_blue)

mycode = prefix +'E_array_blueCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'E_array_blueCUT', data = e_array_blue)

mycode = prefix +'Ee_array_blueCUT = tempfile.NamedTemporaryFile(delete=False)'
exec(mycode)
np.savez(prefix +'Ee_array_blueCUT', data = ee_array_blue)
