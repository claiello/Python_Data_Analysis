...
for index in np.arange(0,len(nametr)):
   ...
   # del se1_dset0
    ...
    red1_dset =np.array(red1_dset0[0:No_experiments[index],initbin:,:,:])/1.0e3 # np.array(red1_dset0,dtype=np.float32)/1.0e3
    del red1_dset0
    gc.collect()
...
##    
    mycode = prefix + str(let[index]) + 'RED1D = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(prefix + str(let[index]) +'RED1D', data = np.nanmean(red1_dset,axis = (0,2,3)))
##    
...
#    mycode =str(prefix + let[index]) + 'BLUE1D = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez(prefix + str(let[index]) +'BLUE1D', data = np.nanmean(red1_dset,axis = (0,2,3)))
#    
    ###############################################################################
    ###############################################################################
    ###############################################################################
