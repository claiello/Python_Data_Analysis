#Short LT scan
#10mus total/400ns moving/8mus decay (1.6mus excitation) 512x512 pixels
...
nametr = ['2016-11-15-1449_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_9',
          '2016-11-15-1452_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_10',
          '2016-11-15-1455_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_11',
          '2016-11-15-1457_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_12',
          '2016-11-15-1500_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_13',
          '2016-11-15-1503_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_14',
          '2016-11-15-1505_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_15',
          '2016-11-15-1508_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_16',
          '2016-11-15-1510_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_17',
          '2016-11-15-1513_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_18']

#SE 2016-11-15-1514_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_19
#
...
let = ['IRT','IIRT','IIIRT','IVRT','VRT','VIRT','VIIRT','VIIIRT','IXRT','XRT']
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    mycode = str(let[index]) + 'SSEchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) + 'SSEchannel', data = se1_dset_cut)
#    
    mycode = str(let[index]) + 'SRedbright = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) +'SRedbright', data = red1_dset_cut_all)
    
    mycode = str(let[index]) + 'SBluebright = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) +'SBluebright', data = blue1_dset_cut_all)
...
##    
    ...
    if do_gmmse_dset:
        ...
        mycode = str(let[index]) +'SSEchannelGMM = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez(str(let[index]) +'SSEchannelGMM', bright = gmmse_se1_bright_dset, means = means, covars = covars, weights = weights)
...
#        calcdecay_each(gmmse_red1_darkse_dset_for_4D[:,start_of_transient::,:,:]/darkse_pct, time_detail= Time_bin*1e-9*fastfactor,titulo='Cathodoluminescence rate decay, bi-exponential fit, \n ' + titulo ,single=False,nominal_time_on=nominal_time_on,fastfactor=fastfactor,other_dset1=red1_dset_cut_all[:,start_of_transient::,:,:]/1.0 ,other_dset2=gmmse_red1_brightse_dset_for_4D[:,start_of_transient::,:,:]/brightse_pct,init_guess=init_guess)    
    ...
    mycode = str(let[index]) +'SZZZBlueDecay = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) +'SZZZBlueDecay', data = gmmse_blue1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    
    mycode = str(let[index]) +'SZZZRedDecay = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) +'SZZZRedDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
...
##files below exist 
#SUM UP IN A CUMULATIVE WAY
b_array_red = np.zeros(10)
be_array_red = np.zeros(10)
e_array_red = np.zeros(10)
ee_array_red = np.zeros(10)
b_array_blue = np.zeros(10)
be_array_blue = np.zeros(10)
e_array_blue = np.zeros(10)
ee_array_blue = np.zeros(10)
blue_int_array = np.zeros(10)
red_int_array = np.zeros(10)
cumu_blue = np.zeros([10,1])
cumu_red = np.zeros([10,1])
...
#for index in listofindex:
    ...
    for jj in np.arange(0,index+1):
        redh = np.load(str(let[jj]) +'SRedbright.npz') 
        redt = redh['data'] + redt
        blueh = np.load(str(let[index]) +'SBluebright.npz') 
        bluet = blueh['data'] + bluet
        ilh = np.load(str(let[index]) +'ILchannel.npz') 
        ilt = ilh['data'] + ilt
        
       
        print(redt.sum())
    
    redt = redt/float(index+1.0)
    bluet = bluet/float(index+1.0)
    ...
    #titulo = 'Region ' + let[index] + '(10kV, 30$\mu$m aperture, 40ns time bins, ' + str(Ps)+ 'nm pixels, blue/red: $</>$ 593nm)'
    titulo = 'Region ' + let[index] + ', ' + str("{0:.6f}".format(np.average(ilt)*1000.0)) + 'mV (10kV, 30$\mu$m aperture, 2$\mu$s time bins, ' + str(Ps)+ 'nm pixels, blue/red: $</>$ 593nm)'
    ...
    #else:
    plt.suptitle("Registration and NO segmentation of cathodoluminescence signal using SE channel, \n" + titulo,fontsize=fsizetit)
    ...
    hlp = np.ones([512,512]) #segmm['bright']
    #hlp[~np.isnan(hlp)] = 1.0 #was 0.0 - this is a hack to count all pixels!!!!!
...
#        hlp[~np.isnan(hlp)] = 1.0
    ...
    #plt.plot(np.arange(0,91)*Time_bin/1e3,np.nanmean(bluen,axis = (1,2)),c='b',label='Blue photons ($<$ 593nm)',lw=3) #in mus, in MHz
     ...
     #origiNAL
    plt.plot(np.arange(0,250)*Time_bin/1e3,np.nanmean(redt*hlp,axis = (0,2,3)),c='r',label='Red photons ($>$ 593nm)',lw=3) #in mus, in MHz
    plt.plot(np.arange(0,250)*Time_bin/1e3,np.nanmean(bluet*hlp,axis = (0,2,3)),c='b',label='Blue photons ($<$ 593nm)',lw=3) #in mus, in MHz
    ...
    datared = np.average(redt, axis = (0))
    datablue = np.average(bluet, axis = (0))
    ...
    init_guess2 = [0.1, 1.0, np.average(datablue[last_pt_offset,:,:]), np.average(datablue[-30,:,:]), 0.005] #e init was 0.5
...
#    
    ...
    if index == 0: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:])+0.1, 0.5, np.nanmean(hlp_red[last_pt_offset,:,:])+0.1, np.nanmean(hlp_red[10:50,:,:]), 10.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:])+0.1, 0.5,np.nanmean(hlp_blue[last_pt_offset,:,:])+0.1, np.nanmean(hlp_blue[10:50,:,:]), 10.0 ] #BLUE
    ...
    if index ==2: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:])+0.1, 0.5, np.nanmean(hlp_red[last_pt_offset,:,:])+0.1, np.nanmean(hlp_red[10:50,:,:]), 10.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:])+0.1,0.5,np.nanmean(hlp_blue[last_pt_offset,:,:])+0.1, np.nanmean(hlp_blue[10:50,:,:]), 10.0 ] #BLUE
   
    if index ==3: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:])+0.1, 0.5, np.nanmean(hlp_red[last_pt_offset,:,:])+0.1, np.nanmean(hlp_red[10:50,:,:]), 10.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:])+0.1, 0.5,np.nanmean(hlp_blue[last_pt_offset,:,:])+0.1, np.nanmean(hlp_blue[10:50,:,:]), 10.0 ] #BLUE
    ...
    if index ==4: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:])+0.1, 0.5, np.nanmean(hlp_red[last_pt_offset,:,:])+0.1, np.nanmean(hlp_red[10:50,:,:]), 10.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:])+0.1, 0.5,np.nanmean(hlp_blue[last_pt_offset,:,:])+0.1, np.nanmean(hlp_blue[10:50,:,:]), 10.0 ] #BLUE
       
    if index ==5: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:])+0.1, 0.5, np.nanmean(hlp_red[last_pt_offset,:,:])+0.1, np.nanmean(hlp_red[10:50,:,:]), 10.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:])+0.1, 0.5,np.nanmean(hlp_blue[last_pt_offset,:,:])+0.1, np.nanmean(hlp_blue[10:50,:,:]), 10.0 ] #BLUE
       
    if index ==6: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:])+0.1, 0.5, np.nanmean(hlp_red[last_pt_offset,:,:])+0.1, np.nanmean(hlp_red[10:50,:,:]), 10.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:])+0.1, 0.5,np.nanmean(hlp_blue[last_pt_offset,:,:])+0.1, np.nanmean(hlp_blue[10:50,:,:]), 10.0 ] #BLUE
       
    if index ==7: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:])+0.1, 0.5, np.nanmean(hlp_red[last_pt_offset,:,:])+0.1, np.nanmean(hlp_red[10:50,:,:]), 10.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:])+0.1, 0.5,np.nanmean(hlp_blue[last_pt_offset,:,:])+0.1, np.nanmean(hlp_blue[10:50,:,:]), 10.0 ] #BLUE
       
    if index ==8: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:])+0.1, 0.5, np.nanmean(hlp_red[last_pt_offset,:,:])+0.1, np.nanmean(hlp_red[10:50,:,:]), 10.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:])+0.1, 0.5,np.nanmean(hlp_blue[last_pt_offset,:,:])+0.1, np.nanmean(hlp_blue[10:50,:,:]), 10.0 ] #BLUE
       
    if index ==9: #needs to be reworked out
       #pass
       init_guess = [np.nanmean(hlp_red[0,:,:])+0.1, 0.5, np.nanmean(hlp_red[last_pt_offset,:,:])+0.1, np.nanmean(hlp_red[10:50,:,:]), 10.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:])+0.1, 0.5,np.nanmean(hlp_blue[last_pt_offset,:,:])+0.1, np.nanmean(hlp_blue[10:50,:,:]), 10.0 ] #BLUE
      
    hlpred = np.nanmean(redt[:,initbin:,:,:]*hlp,axis = (2,3))
    hlpblue = np.nanmean(bluet[:,initbin:,:,:]*hlp,axis = (2,3))
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLred*hlp,axis=(1,2,3)),c='r', label='From red photons ($>$ 700nm)',linestyle='None', marker='o',markersize=4) #in mus, in MHz
   ...
   # major_ticks = [5,10,15,20]
    ...
    multipage_longer('ShortZZZZ-'+ let[index] + '.pdf',dpi=80)
...
Cumu_red = np.load('Cumu_red.npz')
cumu_red = Cumu_red['data']    
Cumu_blue = np.load('Cumu_blue.npz')
cumu_blue = Cumu_blue['data']    
...
plt.rc('font', family='serif')
plt.rc('font', serif='Palatino')    
plt.suptitle("Stefan's UC NPs, heating up",fontsize=fsizetit)
...
#ax1.set_ylim([0,0.1])
#ax1.set_xticks([75,100,125,150,175])
...
ax3.arrow(2, 3, 2, 0, head_width=2, head_length=1, fc='k', ec='k')
ax3.text(2.5, 3.5, 'stage temperature increases', fontsize=15)
#ax4 = ax3.twiny()
...
ax5.set_xlabel('Temperature (C)',fontsize=fsizepl)
#plt.xlim([10,30])
#ax5.set_xticks([75,100,125,150,175])
#plt.ylim([0,80])

multipage_longer('ShortZZZZZSummary.pdf',dpi=80)
