###150kX
# 593 dichroic
#red channel is using red PMT and blue channel using blue PMT
#1200mus total/2mus move/1000mus LT (198mus excite) 512x512 pixels   
#nametr = ['2016-11-15-1227_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_3',
  #        '2016-11-15-1529_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_20',
  #        '2016-11-15-1540_ImageSequence_AndreaSmallRT_150.000kX_10.000kV_30mu_21']
import os
import sys
...
let = ['RT','V300ma','V300mb'] #data not at RT taken at 0.3V
...
##files below exist 
b_array_red = np.zeros(3)
be_array_red = np.zeros(3)
e_array_red = np.zeros(3)
ee_array_red = np.zeros(3)
b_array_blue = np.zeros(3)
be_array_blue = np.zeros(3)
e_array_blue = np.zeros(3)
ee_array_blue = np.zeros(3)
blue_int_array = np.zeros(3)
red_int_array = np.zeros(3)
cumu_blue = np.zeros([3,1])
cumu_red = np.zeros([3,1])
il_data = np.zeros([3,1])

listofindex =np.arange(0,3) # #[0,1,2]  [5] 
#index = 0
#if index is 0:
for index in listofindex:
    ...
    titulo = 'Region ' + let[index] + ', ' + str("{0:.6f}".format(np.average(il['data'])*1000.0)) + 'mV (10kV, 30$\mu$m aperture, 2$\mu$s time bins, ' + str(Ps)+ 'nm pixels [150kX], blue/red: $</>$ 593nm)'
    ...
    hlp = segmm['bright']
    hlp[~np.isnan(hlp)] = 0.0  ####want HACK TO TAKE ALL INTO ACCOUNT???????? use 1.0   #original is 0.0
...
#        hlp[~np.isnan(hlp)] = 1.0
    ...
    if True is False:
        ...
        initbin = 95#102#95
        ##### Achtung!!!!!!!
        # pt 95 is where the decay starts
        # however, data does not fit well, so I'm provisorily cutting the first 7 * 2mus = 14mus of decay
        # in the future, try to fit to a 3 exponential
...
#    
    ...
    if index == 0: #needs to be reworked out
       #init_guess = [np.nanmean(hlp_red[0,:,:]), 5.0, 0.01, 0.6, 50.0 ] #RED
       #init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 5.0,0.01, 1.7, 30.0 ] #BLUE
       
       #init_guess = [np.nanmean(hlp_red[0,:,:]), 1.0, np.nanmean(hlp_red[last_pt_offset:,:,:]), 0.08, 130.0 ] #RED
       #init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 1.0,np.nanmean(hlp_blue[last_pt_offset:,:,:]), 0.2, 50.0 ] #BLUE
       init_guess = [np.nanmean(hlp_red[0,:,:]), 5.0, np.nanmean(hlp_red[last_pt_offset:,:,:]), np.nanmean(hlp_red[10:50,:,:]), 30.0 ] #RED
       init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 5.0,np.nanmean(hlp_blue[last_pt_offset:,:,:]), np.nanmean(hlp_blue[10:50,:,:]), 30.0 ] #BLUE
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLred*hlp,axis=(1,2,3)),c='r', label='From red photons ($>$ 715nm)',linestyle='None', marker='o',markersize=4) #in mus, in MHz
...
plt.rc('font', family='serif')
plt.rc('font', serif='Palatino')    
plt.suptitle("Andrea's UC NPs, 1xRT, 2x0.3V",fontsize=fsizetit)
...
#ax3.arrow(200, 3, 250, 0, head_width=2, head_length=1, fc='k', ec='k')
#ax3.text(250, 3.5, 'stage temperature increases', fontsize=15)
#ax4 = ax3.twiny()
