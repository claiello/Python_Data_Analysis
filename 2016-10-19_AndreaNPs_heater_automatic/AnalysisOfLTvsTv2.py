...
No_experiments = [10,10,10,10,10,10,10,10,10,10]

name = '2016-10-19-0734_ImageSequence_Andrea_10.000kX_10.000kV_30mu_3' 
                  
description = ['Andrea NaYF4:Er'] # (20kV, 30$\mu$m, ' + str(Ps[index]) + 'nm pixels, ' + str(No_experiments[index]) + 'expts., InLens registered)'] #, \n' #+ obs[index] ']    
               
kv = [10]

let = ['I','II','III','IV','V','VI','VII','VIII','IX','X']
...
#for index in np.arange(0,10):
    ...
    il1_dset   = file1['/data/Analog channel 2 : InLens/data'] #10 Scany points X 10 frames X 250 x 250 pixels
    ...
    blue0_dset  = file1['/data/Counter channel 2 : PMT blue/PMT blue time-resolved/data']#10 Scany points X 10 frames x 150 tr pts x250 x 250 pixels
    ...
    red1_dset = red0_dset[index,:,:,:,:]#[0:No_experiments[index]+1,:,:,:]#.reshape((No_experiments[index],:,:,:],150,250,250))
    blue1_dset = blue0_dset[index,:,:,:,:]#[0:No_experiments[index]+1,:,:,:]#.reshape((No_experiments[index],:,:,:],150,250,250))
    
    #convert to smaller data types
    se1_dset2 = np.array(se1_dset[index,:,:,:]) #, dtype=np.float16) #convert to 16 if needed
    #se1_dset2.reshape((10,250,250))
    il1_dset2 = np.array(il1_dset[index,:,:,:]) #, dtype=np.float16)
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    del se1_dset, red1_dset, il1_dset, blue1_dset, red0_dset, blue0_dset
    file1.close()
    gc.collect()
...
cumu_blue = np.zeros([10,10])
cumu_red = np.zeros([10,10])
il_data = np.zeros([10,10])

listofindex =np.arange(0,10) # NUMBER 1 IS BAD, LEAVE IT OUT
#index = 0
#if index is 1:
for index in listofindex:
    ...
    titulo = 'Region ' + let[index] + ', ' + str("{0:.6f}".format(np.average(il['data'])*1000.0)) + 'mV (10kV, 30$\mu$m aperture, 40ns time bins, ' + str(Ps)+ 'nm pixels, blue/red: $</>$ 593nm)'
...
#        hlp[~np.isnan(hlp)] = 1.0
    ...
    if index == 1:
#        init_guess = [144.0, 0.035,0.5,79.0, 1.0 ]
#        init_guess2 = [660.0, 0.045, 0.5,110.0, 1.0 ]
        
        init_guess = [144.0, 0.035,np.nanmean(hlp_red[last_pt_offset:,:,:]), 79.0, 1.0 ]
        init_guess2 = [200.0, 0.045, np.nanmean(hlp_blue[last_pt_offset:,:,:]), 150.0, 0.5 ]

        
    if index == 2:
#        init_guess = [63.0, 0.03, 0.5, 27.0, 5.88 ] #RED
#        init_guess2 = [400.0, 0.04,0.5,33.0, 9.2 ] #BLUE
        
        init_guess = [63.0, 0.03, np.nanmean(hlp_red[last_pt_offset:,:,:]), 27.0, 5.88 ] #RED
        init_guess2 = [400.0, 0.04,np.nanmean(hlp_blue[last_pt_offset:,:,:]),100.0, 17.5 ] #BLUE
                
    if index == 3:
#        init_guess = [50.0, 0.031, 6.2, 7.8, 100.0 ] #RED
#        init_guess2 = [342.0,0.038, 10.0,10.0, 1.0] #BLUE
#        
        init_guess = [50.0, 0.031, np.nanmean(hlp_red[last_pt_offset:,:,:]), 7.8, 1.0 ] #RED
        init_guess2 = [342.0,0.038, np.nanmean(hlp_blue[last_pt_offset:,:,:]),10.0, 1.0] #BLUE
        
    if index == 4:
#        init_guess = [10.0,  0.05,  20.0, 40.0, 10.0 ] #RED
#        init_guess2 = [50.0, 0.05,  50.0, 70.0, 10.0 ] #BLUE
        
        init_guess = [10.0,  0.05,  np.nanmean(hlp_red[last_pt_offset:,:,:]), 40.0, 1.0 ] #RED
        init_guess2 = [50.0, 0.05,  np.nanmean(hlp_blue[last_pt_offset:,:,:]), 10.0, 1.0 ] #BLUE
        
    if index == 5:
#        init_guess = [40.0,  0.05,  15.0, 20.0, 10.0 ] #RED
#        init_guess2 = [350.0, 0.05,  50.0, 70.0, 10.0 ] #BLUE
        
        init_guess = [40.0,  0.05,  np.nanmean(hlp_red[last_pt_offset:,:,:]), 20.0, 1.0 ] #RED
        init_guess2 = [350.0, 0.05,  np.nanmean(hlp_blue[last_pt_offset:,:,:]), 7.0, 1.0 ] #BLUE
        
    if index == 6:
#        init_guess = [20.0,0.05, 4.0,5.0, 5.0 ] #RED
#        init_guess2 = [80.0,0.05, 15.0,20.0, 5.0 ] #BLUE
        
        init_guess = [20.0,0.05, np.nanmean(hlp_red[last_pt_offset:,:,:]),5.0, 5.0 ] #RED
        init_guess2 = [80.0,0.05, np.nanmean(hlp_blue[last_pt_offset:,:,:]),20.0, 5.0 ] #BLUE
    
    if index == 7:
#        init_guess = [12.0,0.05, 5.0,6.0, 5.0 ] #RED
#        init_guess2 = [100.0,0.05, 20.0,25.0, 5.0 ] #BLUE
        
        init_guess = [12.0,0.05, np.nanmean(hlp_red[last_pt_offset:,:,:]),6.0, 5.0 ] #RED
        init_guess2 = [100.0,0.05, np.nanmean(hlp_blue[last_pt_offset:,:,:]),25.0, 5.0 ] #BLUE
        
    if index == 8:
#        init_guess = [10.0,0.05,7.0,8.0, 5.0 ] 
#        init_guess2 = [77.0,0.04, 0.5,32.0, 6.3 ]
        
        init_guess = [10.0,0.05,np.nanmean(hlp_red[last_pt_offset:,:,:]),8.0, 5.0 ] 
        init_guess2 = [77.0,0.04, np.nanmean(hlp_blue[last_pt_offset:,:,:]),32.0, 6.3 ]
        
    if index == 9:
#         init_guess = [12.0,0.05, 8.0,9.0, 5.0]
#         init_guess2 = [60.0,0.05, 25.0,30.0, 5.0]
         
         init_guess = [12.0,0.05, np.nanmean(hlp_red[last_pt_offset:,:,:]),9.0, 5.0]
         init_guess2 = [60.0,0.05, np.nanmean(hlp_blue[last_pt_offset:,:,:]),30.0, 5.0]
...
plt.rc('font', family='serif')
plt.rc('font', serif='Palatino')    
plt.suptitle("Andrea's UC NPs, cooling down",fontsize=fsizetit)
...
plt.plot(np.arange(1,101)*nominal_time_on*fastfactor,cumu_blue.flatten(),c='b', label='From blue photons ($<$ 593nm)',linestyle='None', marker='o',markersize=5)
plt.plot(np.arange(1,101)*nominal_time_on*fastfactor,cumu_red.flatten(),c='r', label='From red photons ($>$ 593nm)',linestyle='None', marker='o',markersize=5)
plt.xlabel("Cumulative e-beam exposure time \n per pixel (nominal, $\mu$s)",fontsize=fsizepl)
...
ax3.arrow(50, 200, 60,0, head_width=2, head_length=6, fc='k', ec='k')
ax3.text(55, 205, 'stage temperature increases', fontsize=15)
#ax4 = ax3.twiny()
...
plt.plot(np.average(x_vec, axis = 1),blue_int_array,c='b', label='From blue photons ($<$ 593nm)',linestyle='None', marker='o',markersize=10)
plt.plot(np.average(x_vec, axis = 1),red_int_array,c='r', label='From red photons ($>$ 593nm)',linestyle='None', marker='o',markersize=10)
plt.legend(loc='best')
plt.ylabel("Average luminescence \n for each experiment, per pixel (KHz)",fontsize=fsizepl)
...
ax5.set_xlim([50,300])
ax5.set_xticks([50,100,150,200,250,300])
