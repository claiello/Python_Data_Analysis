...
#for index in np.arange(12,12):
    ...
    file2   = h5py.File('Long-2016-09-05-1932_ImageSequence_Sigma-UC_63.288kX_10.000kV_30mu_10.hdf5', 'r')   ###Data
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    mycode = 'LONGsingleSEchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('LONGsingleSEchannel', data = se1_dset_cut)
#    
    
    if calc_blue is False: 
        mycode = 'LONGsingleRedbright = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGsingleRedbright', data = red1_dset_cut_all)
    else:
        mycode = 'LONGsingleBluebright = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGsingleBluebright', data = red1_dset_cut_all)
##    
    ...
    if do_gmmse_dset:
        ...
        mycode = 'LONGsingleSEchannelGMM = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGsingleSEchannelGMM', bright = gmmse_se1_bright_dset, means = means, covars = covars, weights = weights)
...
#
    ...
    if calc_blue is False:
        mycode = 'LONGsingleZZZRedDecay = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGsingleZZZRedDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    else:
        mycode = 'LONGsingleZZZBlueDecay = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGsingleZZZBlueDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    
    multipage('LONGsingleZZZ.pdf',dpi=80)
...
init_guess = [np.average(datared[0,:,:]), 0.5, np.average(datared[last_pt_offset,:,:]), np.average(datared[-15,:,:]), 2.0] #e init was 0.5
