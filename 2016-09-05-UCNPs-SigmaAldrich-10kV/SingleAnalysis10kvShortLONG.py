...
nominal_time_on = 15 #time during which e-beam nominally on, in mus
totalpoints = 62 #total number of time-resolved points
### data
name = ['Double.hdf5', ]
...
ap = ['30']
Pixel_size = [1.8e-09]
Ps = [1.8] #pixel size in nm, the numbers above with round nm precision
...
#for index in np.arange(12,12):
    ...
    file2   = h5py.File('ShortLong-2016-09-05-1921_ImageSequence_Sigma-UC_63.288kX_10.000kV_30mu_9.hdf5', 'r')   ###Data
    #file3   = h5py.File('PentaZoom50expts.hdf5', 'r') 
    titulo =  'Upconverting NPs (10kV, 30$\mu$m aperture, 1$\mu$s time bins, 63kX or 1.8nm pixels)'
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    mycode = 'ShortLONGsingleSEchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('ShortLONGsingleSEchannel', data = se1_dset_cut)
#    
    
    if calc_blue is False: 
        mycode = 'ShortLONGsingleRedbright = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('ShortLONGsingleRedbright', data = red1_dset_cut_all)
    else:
        mycode = 'ShortLONGsingleBluebright = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('ShortLONGsingleBluebright', data = red1_dset_cut_all)
##    
    ...
    if do_gmmse_dset:
        ...
        mycode = 'ShortLONGsingleSEchannelGMM = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('ShortLONGsingleSEchannelGMM', bright = gmmse_se1_bright_dset, means = means, covars = covars, weights = weights)
...
#
    ...
    if calc_blue is False:
        mycode = 'ShortLONGsingleZZZRedDecay = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('ShortLONGsingleZZZRedDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    else:
        mycode = 'ShortLONGsingleZZZBlueDecay = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('ShortLONGsingleZZZBlueDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    
    multipage('ShortLONGsingleZZZ.pdf',dpi=80)
...
### 593 dichroic
se = np.load('ShortLONGsingleSEchannel.npz') 
segmm = np.load('ShortLONGsingleSEchannelGMM.npz') 
red = np.load('ShortLONGsingleRedbright.npz') 
blue = np.load('ShortLONGsingleBluebright.npz') 
...
titulo =  '150nm upconverting NPs (10kV, 30$\mu$m aperture, 1$\mu$s time bins, 63kX or 1.8nm pixels)'
...
hlp = segmm['bright']
hlp[~np.isnan(hlp)] = 1.0
plt.plot(np.arange(0,62)*Time_bin/1e3,np.nanmean(red['data']*hlp,axis = (0,2,3)),c='r',label='Red photons ($>$ 593nm)',lw=3) #in mus, in MHz
plt.plot(np.arange(0,62)*Time_bin/1e3,np.nanmean(blue['data']*hlp,axis = (0,2,3)),c='b',label='Blue photons ($<$ 593nm)',lw=3) #in mus, in MHz
#plt.plot(np.arange(0,62)*Time_bin/1e3,np.average(red['data'],axis = (0,2,3)),c='r',label='Red photons ($>$ 593nm)',lw=3) #in mus, in MHz
#plt.plot(np.arange(0,62)*Time_bin/1e3,np.average(blue['data'],axis = (0,2,3)),c='b',label='Blue photons ($<$ 593nm)',lw=3) #in mus, in MHz
ax1.axvspan(1,2, alpha=0.25, color='yellow')
unit = 'kHz'
...
init_guess = [np.average(datared[0,:,:]), 0.5, np.average(datared[last_pt_offset,:,:]), np.average(datared[-15,:,:]), 0.1] #e init was 0.5
init_guess = [np.average(datared[0,:,:]), 0.93, np.average(datared[last_pt_offset,:,:]), np.average(datared[-15,:,:]), 1.0] #e init was 0.5
