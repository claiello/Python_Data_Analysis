...
ap = ['30']
Pixel_size = [7.1e-9] #according to fiji <-   according to txt file, probably wrong -> 1.8e-09]
Ps = [7.1]
nametr = ['']
...
#for index in listofindex:
    ...
    se = np.load('LONGsingleSEchannel.npz') 
    segmm = np.load('LONGsingleSEchannelGMM.npz') 
    red = np.load('LONGsingleRedbright.npz') 
    blue = np.load('LONGsingleBluebright.npz') 
    ...
    titulo = 'UCNP commercial' + let[index] + ' (10kV, 30$\mu$m aperture, 1$\mu$s time bins, ' + str(Ps)+ 'nm pixels, red: all light)'
...
#    cb2 = plt.colorbar(im, cax = axColor, orientation="horizontal",label='Photon counts ' + unit) 
    ...
    ax1.set_title('CL data averaged over time \n and 50 experiments',fontsize=fsizepl)
    imb = plt.imshow(np.nanmean(red['data']+blue['data'],axis = (0,1)),cmap=cm.Reds) #or 'OrRd'
    sbar = sb.AnchoredScaleBar(ax1.transData, length_scalebar_in_pixels, scalebar_legend, style = 'dark', loc = 4)
    ax1.add_artist(sbar)
    plt.axis('off')
    ...
    plt.plot(np.arange(0,76)*Time_bin/1e3,np.nanmean((red['data']+blue['data'])*hlp,axis = (0,2,3)),c='r',lw=2) #in mus, in MHz
    plt.plot(np.arange(0,76)*Time_bin/1e3,np.nanmean((red['data']+blue['data'])*hlpd,axis = (0,2,3)),c='DarkRed',lw=2) #in mus, in MHz
    ...
    datared = np.average((red['data']+blue['data']), axis = (0))
    #datablue = np.average(blue['data'], axis = (0))
    
    if True is False:
        pass
    else:
        initbin = (16+1)-1 #init bin for decay
        backgdinit = 1
        ### 700ns /40ns = 7. ....
   ...
   # cinitb = np.nanmean(datablue_init*hlp,axis=(0,1,2))
    ...
    b,e,be,ee,b2,e2,be2,ee2 = calcdecay_subplot_nan(datared*hlp, time_detail= Time_bin*1e-9*fastfactor,titulo='',single=False,other_dset1=None,other_dset2=None,init_guess=init_guess,init_guess2=None,unit='kHz',error_array=None) #,error_array=error_arrayr, error_array2=error_arrayb)    
...
#
    ...
    multipage_longer('ZZZZSumChannels-'+ let[index] + '.pdf',dpi=80)
