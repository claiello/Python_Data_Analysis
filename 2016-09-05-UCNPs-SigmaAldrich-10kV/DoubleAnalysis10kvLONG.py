...
#for index in np.arange(12,12):
    ...
    file1    = h5py.File('SE-2016-09-05-1752_ImageSequence_Sigma-UC_103.168kX_10.000kV_30mu_4.hdf5', 'r')  ###SE
    file2   = h5py.File('SS-2016-09-05-1831_ImageSequence_Sigma-UC_103.168kX_10.000kV_30mu_6.hdf5', 'r')   ###Data
    #file3   = h5py.File('PentaZoom50expts.hdf5', 'r') 
    titulo =  'Upconverting NPs (10kV, 30$\mu$m aperture, 1$\mu$s time bins, 103kX or 1.1nm pixels)'
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    mycode = 'LONGDoubleSEchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('LONGDoubleSEchannel', data = se1_dset_cut)
#    
    
    if calc_blue is False: 
        mycode = 'LONGDoubleRedbright = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGDoubleRedbright', data = red1_dset_cut_all)
    else:
        mycode = 'LONGDoubleBluebright = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGDoubleBluebright', data = red1_dset_cut_all)
##    
    ...
    if do_gmmse_dset:
        ...
        mycode = 'LONGDoubleSEchannelGMM = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGDoubleSEchannelGMM', bright = gmmse_se1_bright_dset, means = means, covars = covars, weights = weights)
...
#
    ...
    if calc_blue is False:
        mycode = 'LONGDoubleZZZRedDecay = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGDoubleZZZRedDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    else:
        mycode = 'LONGDoubleZZZBlueDecay = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('LONGDoubleZZZBlueDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    
    multipage('LONGDoubleZZZ.pdf',dpi=80)
...
titulo =  '150nm upconverting NPs (10kV, 30$\mu$m aperture, 1$\mu$s time bins, 103kX or 1.1nm pixels)'
