...
#No_pixels = 250
Time_bin = 1000#in ns; 1/clock of 25MHz 
nominal_time_on = 15.0 #time during which e-beam nominally on, in mus
totalpoints = 76 #total number of time-resolved points
### data
...
Ps = [1.1]
nametr = ['']
No_experiments = 50.0*np.ones(len(nametr))

description = ['Commercial NDs'] # (20kV, 30$\mu$m, ' + str(Ps[index]) + 'nm pixels, ' + str(No_experiments[index]) + 'expts., InLens registered)'] #, \n' #+ obs[index] ']    
...
listofindex =np.arange(0,1) # #[0,1,2]  [5]  ###goes until 17
index = 0
if index is 0:
#for index in listofindex:
    ...
    se = np.load('LONGDoubleSEchannel.npz') 
    segmm = np.load('LONGDoubleSEchannelGMM.npz') 
    red = np.load('LONGDoubleRedbright.npz') 
    blue = np.load('LONGDoubleBluebright.npz') 
    ...
    titulo = 'UCNP commercial' + let[index] + ' (10kV, 30$\mu$m aperture, 1$\mu$s time bins, ' + str(Ps)+ 'nm pixels, 593nm dichroic)'
    ...
    hlp[np.isnan(hlp)] = 1.0 #outside   #to consider all light, this should be 1.0; to consider only inside, this should be 0.0
    ...
    plt.plot(np.arange(0,76)*Time_bin/1e3,np.nanmean(red['data']*hlp,axis = (0,2,3)),c='r',lw=2) #in mus, in MHz
    plt.plot(np.arange(0,76)*Time_bin/1e3,np.nanmean(red['data']*hlpd,axis = (0,2,3)),c='DarkRed',lw=2) #in mus, in MHz
    plt.plot(np.arange(0,76)*Time_bin/1e3,np.nanmean(blue['data']*hlp,axis = (0,2,3)),c='b',lw=2) #in mus, in MHz
    plt.plot(np.arange(0,76)*Time_bin/1e3,np.nanmean(blue['data']*hlpd,axis = (0,2,3)),c='DarkBlue',lw=2) #in mus, in MHz
    ax1.axvspan(1.0+0.3,1.0+15.0+0.3, alpha=0.25, color='yellow')
    unit = 'kHz'
    plt.ylabel("Average luminescence \n of each time bin, per signal pixel (" + unit + ")",fontsize=fsizepl)
    plt.xlabel("Behaviour of e-beam during each experiment: \n 15-ON + 60-OFF ($\mu$s)",fontsize=fsizepl)
    ...
    if True is False:
        ...
        initbin = (16+1)-1
        ### 700ns /40ns = 7. ....
        datared_init = datared[0:1,:,:]
        datared = datared[initbin:,:,:]
        datablue_init = datablue[0:1,:,:]
    ...
    init_guessb = calc_double_fit(np.arange(0,datablue.shape[0])*Time_bin*1e-9*fastfactor,np.nanmean(datablue*hlp,axis=(1,2)),dt= Time_bin*1e-9*fastfactor,cut_long = 6,cut_short = 4)
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLred*hlpd,axis=(1,2,3)),c='DarkRed', label='',linestyle='None', marker='o',markersize=3) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLred*hlpd,axis=(0,1,2,3))*np.ones(dataALLred.shape[0]),c='DarkRed',  linewidth= lw) #in mus, in MHz
    #init red intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLred[:,0:1,:,:]*hlp,axis=(1,2,3)),c='r', label='',linestyle='None', marker='o',markersize=3) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLred[:,0:1,:,:]*hlp,axis=(0,1,2,3))*np.ones(dataALLred.shape[0]),'--',c='r',  linewidth= lw) #in mus, in MHz


    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue*hlp,axis=(1,2,3)),c='b', label='From photons $>$ 509nm',linestyle='None', marker='o',markersize=4) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue*hlp,axis=(0,1,2,3))*np.ones(dataALLred.shape[0]),c='b',  linewidth= lw) #in mus, in MHz
    #background intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue*hlpd,axis=(1,2,3)),c='DarkBlue', label='',linestyle='None', marker='o',markersize=3) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue*hlpd,axis=(0,1,2,3))*np.ones(dataALLred.shape[0]),c='DarkBlue',  linewidth= lw) #in mus, in MHz
    #init red intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue[:,0:1,:,:]*hlp,axis=(1,2,3)),c='b', label='',linestyle='None', marker='o',markersize=3) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue[:,0:1,:,:]*hlp,axis=(0,1,2,3))*np.ones(dataALLred.shape[0]),'--',c='b',  linewidth= lw) #in mus, in MHz
    ...
    sig_to_initred[index] =   np.nanmean(dataALLred*hlp,axis=(0,1,2,3))/np.nanmean(dataALLred[:,0:1,:,:]*hlp,axis=(0,1,2,3))
    
    
    sig_to_back_blue[index] =   np.nanmean(dataALLblue*hlp,axis=(0,1,2,3))/np.nanmean(dataALLblue*hlpd,axis=(0,1,2,3))
    sig_to_initblue[index] =   np.nanmean(dataALLblue*hlp,axis=(0,1,2,3))/np.nanmean(dataALLblue[:,0:1,:,:]*hlp,axis=(0,1,2,3))
    
    multipage_longer('ZZZZDouble-'+ let[index] + '.pdf',dpi=80)
