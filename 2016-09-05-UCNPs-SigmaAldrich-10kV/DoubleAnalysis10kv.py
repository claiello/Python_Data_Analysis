...
#for index in np.arange(12,12):
    ...
    file2   = h5py.File('NImp-2016-09-05-1813_ImageSequence_Sigma-UC_103.168kX_10.000kV_30mu_5.hdf5', 'r')   ###Data
    #file3   = h5py.File('PentaZoom50expts.hdf5', 'r') 
    titulo =  'Upconverting NPs (10kV, 30$\mu$m aperture, 40ns time bins, 103kX or 1.1nm pixels)'
...
#    red1_dset_reg = np.array(new, dtype=np.float32)
    ...
    mycode = 'DoubleSEchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez('DoubleSEchannel', data = se1_dset_cut)
#    
    
    if calc_blue is False: 
        mycode = 'DoubleRedbright = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('DoubleRedbright', data = red1_dset_cut_all)
    else:
        mycode = 'DoubleBluebright = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('DoubleBluebright', data = red1_dset_cut_all)
##    
    ...
    if do_gmmse_dset:
        ...
        mycode = 'DoubleSEchannelGMM = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('DoubleSEchannelGMM', bright = gmmse_se1_bright_dset, means = means, covars = covars, weights = weights)
...
#
    ...
    if calc_blue is False:
        mycode = 'DoubleZZZRedDecay = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('DoubleZZZRedDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    else:
        mycode = 'DoubleZZZBlueDecay = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez('DoubleZZZBlueDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
    
    multipage('DoubleZZZ.pdf',dpi=80)
...
### 593 dichroic
se = np.load('DoubleSEchannel.npz') 
segmm = np.load('DoubleSEchannelGMM.npz') 
red = np.load('DoubleRedbright.npz') 
blue = np.load('DoubleBluebright.npz') 
...
titulo =  '150nm upconverting NPs (10kV, 30$\mu$m aperture, 40ns time bins, 103kX or 1.1nm pixels)'
