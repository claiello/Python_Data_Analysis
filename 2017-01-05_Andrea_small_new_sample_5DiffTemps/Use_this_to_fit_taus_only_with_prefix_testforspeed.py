...
prefixend = prefix
#prefixend = 'fixCsametaudouble_nodiv'
#prefixend = 'fixCprevtaudouble_nodiv'
#prefixend = 'varCsametaudouble_nodiv'
#prefixend = 'varCprevtaudouble_nodiv'
...
for index in [1,2,4,5]: #listofindex:
    ...
    init_guess = calc_double_fit(np.arange(0,red.shape[0])*Time_bin*1.0e-9*fastfactor,red,dt= Time_bin*1.0e-9*fastfactor,cut_long=cut_longa,cut_short=cut_shorta)
    
    gc.collect()    
    
    init_guessb = calc_double_fit(np.arange(0,blue.shape[0])*Time_bin*1.0e-9*fastfactor,blue,dt= Time_bin*1.0e-9*fastfactor,cut_long=cut_longa,cut_short=cut_shorta)
...
#        init_guessb[1] = 5.0
    ...
    if (prefix == 'fixCprevtaudouble') or (prefix=='varCprevtaudouble'):
        ...
        if index == 1:
            ...
            print('doing its thing')
            if index == 2:
                init_guess[1] = np.average(b_array_red[1])
                init_guess[4] = np.average(e_array_red[1])
                init_guessb[1] = np.average(b_array_blue[1])
                init_guessb[4] = np.average(e_array_blue[1])  
            if index == 4:
                init_guess[1] = np.average(b_array_red[2]+b_array_red[1])
                init_guess[4] = np.average(e_array_red[2]+e_array_red[1])
                init_guessb[1] = np.average(b_array_blue[2]+b_array_blue[1])
                init_guessb[4] = np.average(e_array_blue[2]+e_array_blue[1])  
            if index == 5:
                init_guess[1] = np.average(b_array_red[2]+b_array_red[1]+b_array_red[4])
                init_guess[4] = np.average(e_array_red[2]+e_array_red[1]+e_array_red[4])
                init_guessb[1] = np.average(b_array_blue[2]+b_array_blue[1]+b_array_blue[4])
                init_guessb[4] = np.average(e_array_blue[2]+e_array_blue[1]+e_array_blue[4])  
    ...
    b,e,be,ee,b2,e2,be2,ee2,chisquared,chisquared2 = calcdecay_subplot_nan_1D(red, time_detail= Time_bin*1.0e-9*fastfactor,titulo='',single=False,other_dset1=None,other_dset2=blue,init_guess=init_guess,init_guess2=init_guessb,unit='kHz',error_array=None) #,error_array=error_arrayr, error_array2=error_arrayb)    
