...
#### REal RT, no voltage turned on at any point
...
for index in np.arange(0,1):
    ...
    #red0_dset0  = file2['/data/Counter channel 1 : PMT red/PMT red time-resolved/data']#10 Scany points X10 frames x 150 tr pts x250 x 250 pixels
    ...
    red0_dset0  = file2['/data/Counter channel 2 : PMT blue/PMT blue time-resolved/data']#10 Scany points X 10 frames x 150 tr pts x250 x 250 pixels
    ...
    Pixel_size = red0_dset0.attrs['element_size_um'][1]*1000.0 #saved attribute is in micron; converting to nm
    Ps = [str("{0:.2f}".format(Pixel_size))] #pixel size in nm, the numbers above with round nm precision    
    
    print(red0_dset0.shape)
    ...
    se1_dset2 = se1_dset0[chosenexp,:,:]
    se1_dset2 = se1_dset2.reshape(1,se1_dset2.shape[0],se1_dset2.shape[1])
    red1_dset = red0_dset0[chosenexp,:,:,:]/1.0e3#.reshape((No_experiments[index],:,:,:],150,250,250))
    red1_dset = red1_dset.reshape(1,red1_dset.shape[0],red1_dset.shape[1],red1_dset.shape[2])
    ...
    il1_dset2 = np.array(il1_dset[chosenexp,:,:]) #, dtype=np.float16)
    il1_dset2 = il1_dset2.reshape(1,il1_dset2.shape[0],il1_dset2.shape[1])
...
#        
    ...
    mycode = str(let[index]) + 'ChosenExp' + str(chosenexp) + 'ILchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) + 'ChosenExp' + str(chosenexp) + 'ILchannel', data = il1_dset2)
    print(il1_dset2.shape)
    print(np.average(il1_dset2, axis=(0,1,2)))
    del il1_dset2
    gc.collect()
...
#    ylen = se1_dset_reg.shape[1]
    se1_dset_cut = se1_dset_reg#[np.floor(xlen/2)-xval+delx:np.floor(xlen/2)+xval+delx,np.floor(ylen/2)-yval+dely:np.floor(ylen/2)+yval+dely]
    red1_dset_cut = red1_dset_reg#[:,np.floor(xlen/2)-xval+delx:np.floor(xlen/2)+xval+delx,np.floor(ylen/2)-yval+dely:np.floor(ylen/2)+yval+dely]
    red1_dset_cut_all = red1_dset_reg_all#[:,:,np.floor(xlen/2)-xval+delx:np.floor(xlen/2)+xval+delx,np.floor(ylen/2)-yval+dely:np.floor(ylen/2)+yval+dely]
...
#    blue1_dset_cut_all = np.array(blue1_dset_cut_all, dtype=np.float32)
    ...
    mycode = str(let[index]) + 'ChosenExp' + str(chosenexp)+ 'SEchannel = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) + 'ChosenExp' + str(chosenexp)+ 'SEchannel', data = se1_dset_cut)
#    
#    mycode = str(let[index]) + 'ChosenExp' + str(chosenexp)+ 'Redbright = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez(str(let[index]) + 'ChosenExp' + str(chosenexp)+'Redbright', data = red1_dset_cut_all)
#    
    mycode = str(let[index]) + 'ChosenExp' + str(chosenexp)+ 'Bluebright = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) + 'ChosenExp' + str(chosenexp)+'Bluebright', data = red1_dset_cut_all)
...
#    
    ...
    if do_gmmse_dset:
        ...
        mycode = str(let[index]) + 'ChosenExp' + str(chosenexp)+'SEchannelGMM = tempfile.NamedTemporaryFile(delete=False)'
        exec(mycode)
        np.savez(str(let[index]) + 'ChosenExp' + str(chosenexp)+'SEchannelGMM', bright = gmmse_se1_bright_dset, means = means, covars = covars, weights = weights)
...
#        calcdecay_each(gmmse_red1_darkse_dset_for_4D[:,start_of_transient::,:,:]/darkse_pct, time_detail= Time_bin*1e-9*fastfactor,titulo='Cathodoluminescence rate decay, bi-exponential fit, \n ' + titulo ,single=False,nominal_time_on=nominal_time_on,fastfactor=fastfactor,other_dset1=red1_dset_cut_all[:,start_of_transient::,:,:]/1.0 ,other_dset2=gmmse_red1_brightse_dset_for_4D[:,start_of_transient::,:,:]/brightse_pct,init_guess=init_guess)    
    ...
    mycode = str(let[index]) + 'ChosenExp' + str(chosenexp)+'ZZZBlueDecay = tempfile.NamedTemporaryFile(delete=False)'
    exec(mycode)
    np.savez(str(let[index]) + 'ChosenExp' + str(chosenexp)+'ZZZBlueDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
#    
#    mycode = str(let[index]) + 'ChosenExp' + str(chosenexp)+'ZZZRedDecay = tempfile.NamedTemporaryFile(delete=False)'
#    exec(mycode)
#    np.savez(str(let[index]) + 'ChosenExp' + str(chosenexp)+'ZZZRedDecay', data = gmmse_red1_darkse_dset[start_of_transient::,:,:]/darkse_pct)
#    
...
##files below exist 
klklklk
b_array_red = np.zeros(5)
be_array_red = np.zeros(5)
e_array_red = np.zeros(5)
ee_array_red = np.zeros(5)
b_array_blue = np.zeros(5)
be_array_blue = np.zeros(5)
e_array_blue = np.zeros(5)
ee_array_blue = np.zeros(5)
blue_int_array = np.zeros(5)
red_int_array = np.zeros(5)
cumu_blue = np.zeros([5,1])
cumu_red = np.zeros([5,1])
il_data = np.zeros([5,1])

listofindex =np.arange(0,5) # #[0,1,2]  [5] 
index = 0
if index is 1:
#for index in listofindex:
    ...
    titulo = 'Region ' + let[index] + ' (10kV, 30$\mu$m aperture, 1$\mu$s time bins, ' + str(Ps)+ 'nm pixels, blue/red photons: $</>$ 593nm)'
    ...
    #else:
    plt.suptitle("No Registration, no segmentation of cathodoluminescence signal using SE channel, !INIT BIN 203 NOT 200/201!, \n" + titulo,fontsize=fsizetit)
    ...
    ax1.set_title('Blue photons',fontsize=fsizepl)
    ...
    cb1 = plt.colorbar(im, cax = axColor, orientation="horizontal",label='Counts ' + unit,ticks=[0,np.nanmax(blue['data']/2.0),np.nanmax(blue['data'])])
...
#    labels_ws = watershed(-distance, markers, mask=image)
    ...
    ax1.set_title('Red photons',fontsize=fsizepl)
    ...
    axColor = plt.axes([box.x0, box.y0*1.025 , box.width,0.01 ])    
    unit = '(kHz)'
    cb1 = plt.colorbar(im, cax = axColor, orientation="horizontal",label='Counts ' + unit,ticks=[0,np.nanmax(red['data']/2.0),np.nanmax(red['data'])])
...
#        hlp[~np.isnan(hlp)] = 1.0
    ...
    hlp = 1.0
    plt.plot(np.arange(0,1700)*Time_bin/1e3,np.nanmean(red['data']*hlp,axis = (0,2,3)),c='r',label='Red photons ($>$ 593nm)',lw=3) #in mus, in MHz
    plt.plot(np.arange(0,1700)*Time_bin/1e3,np.nanmean(blue['data']*hlp,axis = (0,2,3)),c='b',label='Blue photons ($<$ 593nm)',lw=3) #in mus, in MHz
    ...
    if True is False:
        ...
        initbin = 203   ####"correct" IS 200 OR 201 (BC OF 300NS DELAY)
        timeinit = 50
        datared_init = datared[0:timeinit-2,:,:]
        datablue_init = datared[0:timeinit-2,:,:]
    ...
    fastfactor = 1
    last_pt_offset = -500 #sometimes use -1, last point, but sometimes this gives 0. -10 seems to work
...
#        init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 15.0,np.nanmean(hlp_blue[last_pt_offset:,:,:]), np.nanmean(hlp_blue[10:50,:,:]), 200.0 ] #BLUE
#    else:
    cguessred=np.nanmean(datared_init*hlp,axis=(0,1,2))
    cguessblue = np.nanmean(datablue_init*hlp,axis=(0,1,2))
    init_guess = [np.nanmean(hlp_red[0,:,:]), 15.0, cguessred, np.nanmean(hlp_red[-1000:-500,:,:]), 200.0 ] #RED
    init_guess2 = [np.nanmean(hlp_blue[0,:,:]), 15.0,cguessblue, np.nanmean(hlp_blue[-1000:-500,:,:]), 200.0 ] #BLUE 
    ...
    plt.semilogy(xx_array/1e-6,np.nanmean(datared_init*hlp,axis=(0,1,2))*np.ones(len(xx_array)),'r--',lw=2,label='Background for red photons, before e-beam')
    plt.semilogy(xx_array/1e-6,np.nanmean(datablue_init*hlp,axis=(0,1,2))*np.ones(len(xx_array)),'b--',lw=2,label='Background for blue phtoons, before e-beam')
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue*hlp,axis=(1,2,3)),c='b',linestyle='None', marker='o',markersize=4) #in mus, in MHz
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLred*hlp,axis=(1,2,3)),c='r', linestyle='None', marker='o',markersize=4) #in mus, in MHz
...
plt.rc('font', family='serif')
plt.rc('font', serif='Palatino')    
fig41.suptitle('Lifetimes fitted after 1500$\mu$s decay, small NaYF$_4$ CS from Andrea \n (10kV, 30$\mu$m aperture, 1$\mu$s time bins)',fontsize=fsizetit)     

ax2 = plt.subplot2grid((2,2), (0,0), colspan=1, rowspan=1)
ax1 = plt.subplot2grid((2,2), (1,0), colspan=1, sharex=ax2)
...
#ax1.set_xlabel('Temperature on heater (C)')
#x_vec = KThermocouplerconversion(np.mean(il_data[il_data > 0.0])) 
x_vec = [0, 0.25,0.5, 0.75, 1] #(6,10)
...
ax1.set_xlabel('Voltage (V) [$\propto$ T]',fontsize=fsizepl)
...
#plt.plot(np.arange(1,3+1)*nominal_time_on*fastfactor,cumu_blue.flatten(),c='b', label='From blue photons ($<$ 593nm)',linestyle='None', marker='o',markersize=5)
#plt.plot(np.arange(1,3+1)*nominal_time_on*fastfactor,cumu_red.flatten(),c='r', label='From red photons ($>$ 593nm)',linestyle='None', marker='o',markersize=5)
#plt.xlabel("Cumulative e-beam exposure time \n per pixel (nominal, $\mu$s)",fontsize=fsizepl)
...
#ax3.text(25, 78, 'stage temperature increases', fontsize=15)
#ax4 = ax3.twiny()
#ax3Ticks = ax3.get_xticks()   
#ax4Ticks = ax3Ticks
#ax4.set_xticks(ax4Ticks)
#ax4.set_xbound(ax3.get_xbound())
#ax4.set_xticklabels(x_vec)
...
plt.plot(x_vec,red_int_array/blue_int_array,c='k', label='Ratio intensity red/blue photons',linestyle='None', marker='o',markersize=10)
plt.legend(loc='best')
plt.ylabel("Average luminescence \n for each experiment, per pixel (KHz)",fontsize=fsizepl)
...
ax5.set_xlabel('Voltage (V) [$\propto$ T]',fontsize=fsizepl)
plt.xlim([-0.5,1.5])
