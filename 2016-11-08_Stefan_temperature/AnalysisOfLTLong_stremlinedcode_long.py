...
for index in listofindex:
    ...
    length_scalebar = 1000.0 #in nm 
    scalebar_legend = '1000nm'
    length_scalebar_in_pixels = np.ceil(length_scalebar/(Pixel_size[index]))
    
    titulo = description + str("{0:.6f}".format(np.average(il['data'])*1000.0)) + 'mV (10kV, 30$\mu$m aperture, 2$\mu$s time bins, ' + str(Ps)+ 'nm pixels, blue/red: $</>$ 593nm)'
    ...
    hlpse[np.isnan(hlpse)] = 0.0 #outside #all light considered
    ...
    hlp[np.isnan(hlp)] = 1.0 #outside   #all light considered
    
    # OUTSIDE
    hlpd = np.copy(segmm['bright'])
    hlpd[~np.isnan(hlpd)] = 0.0 
    hlpd[np.isnan(hlpd)] = 0.0  #all light considered
    ...
    if True is False:
        ...
        initbin = (95+6)-1 #init bin for decay
        backgdinit = 1
        ### 700ns /40ns = 7. ....
...
#    ee_array_blue[index] = ee2  
    ...
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLred[:,backgdinit:initbin,:,:]*hlpd,axis=(1,2,3)),c='DarkRed', linestyle='None', marker='o',markersize=3) #in mus, in MHz
    ...
    #background intensity
    plt.plot(np.arange(1,No_experiments[index]+1)*nominal_time_on*fastfactor,np.nanmean(dataALLblue[:,backgdinit:initbin,:,:]*hlpd,axis=(1,2,3)),c='DarkBlue',linestyle='None', marker='o',markersize=3) #in mus, in MHz
